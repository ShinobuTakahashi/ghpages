(function (global, factory) {
	...
	constants$2.PSEUDO_INFINITE_AGE = constants$2.DEFAULT_POLLING_TICK_THRESHOLD = constants$2.DEFAULT_JUMP_IGNORE_THRESHOLD = constants$2.DEFAULT_JUMP_TRY_THRESHOLD = constants$2.DEFAULT_SKIP_THRESHOLD = constants$2.DEFAULT_SKIP_TICKS_AT_ONCE = constants$2.DEFAULT_DELAY_IGNORE_THRESHOLD = void 0;
	/**
	 * 遅延を無視する域値のデフォルト。
	 * `LoopConfiguration#delayIgnoreThreshold` のデフォルト値。
	 * このフレーム以下の遅延は遅れてないものとみなす(常時コマが飛ぶのを避けるため)。
	 */
	constants$2.DEFAULT_DELAY_IGNORE_THRESHOLD = 6;
	/**
	 * 「早送り」時倍率のデフォルト値。
	 * `LoopConfiguration#skipTicksAtOnce` のデフォルト値。
	 */
	constants$2.DEFAULT_SKIP_TICKS_AT_ONCE = 100;
	/**
	 * 「早送り」状態に移る域値のデフォルト。
	 * `LoopConfiguration#skipThreshold` のデフォルト値。
	 */
	constants$2.DEFAULT_SKIP_THRESHOLD = 100;
	/**
	 * スナップショットジャンプを試みる域値のデフォルト。
	 * `LoopConfiguration#jumpTryThreshold` のデフォルト値。
	 */
	constants$2.DEFAULT_JUMP_TRY_THRESHOLD = 30000; // 30FPSの100倍早送りで換算3000FPSで進めても10秒かかる閾値
	/**
	 * 取得したスナップショットを無視する域値のデフォルト。
	 * `LoopConfiguration#jumpIgnoreThreshold` のデフォルト値。
	 */
	constants$2.DEFAULT_JUMP_IGNORE_THRESHOLD = 15000; // 30FPSの100倍早送りで換算3000FPSで進めて5秒で済む閾値
	/**
	 * 最新ティックをポーリングする間隔(ms)のデフォルト。
	 */
	constants$2.DEFAULT_POLLING_TICK_THRESHOLD = 10000;
	/**
	 * 擬似的に無限未来として扱うage。
	 */
	constants$2.PSEUDO_INFINITE_AGE = 1892160000; // = 365 * 86400 * 60 = 60FPSで一年分。(特に制限ではないが32bit signed intに収まる)
	...
	var MemoryAmflowClient$1 = {};

	Object.defineProperty(MemoryAmflowClient$1, "__esModule", {
	  value: true
	});
	MemoryAmflowClient$1._cloneDeep = MemoryAmflowClient$1.MemoryAmflowClient = void 0;
	var MemoryAmflowClient = /** @class */function () {
	  ...
	  MemoryAmflowClient.prototype.sendTick = function (tick) {
	    ...
	    if (!this._tickList) {
	      ...
	      if (this._tickList[0 /* From */] <= tick[0 /* Frame */] && tick[0 /* Frame */] <= this._tickList[1 /* To */]) throw new Error("illegal age tick");
	    ...
	    if (events || storageData) {
	      if (events) {
	        tick[1 /* Events */] = events.filter(function (event) {
	          return !(event[1 /* EventFlags */] & 8 /* Transient */);
	        });
	      }
	      // @ts-ignore
	  ...
	  MemoryAmflowClient.prototype.getStartPoint = function (opts, callback) {
	    var _this = this;
	    setTimeout(function () {
	      if (!_this._startPoints || _this._startPoints.length === 0) return void callback(new Error("no startpoint"));
	      var index = 0;
	      if (opts.frame != null) {
	...
	MemoryAmflowClient$1.MemoryAmflowClient = MemoryAmflowClient;
	  ...
	  if (v && _typeof(v) === "object") {
	    if (Array.isArray(v)) {
	      ...
	      return Object.keys(v).reduce(function (acc, k) {
	        return acc[k] = _cloneDeep(v[k]), acc;
	      }, {});
	    }
	  }
	  return v;
	}
	MemoryAmflowClient$1._cloneDeep = _cloneDeep;

	var ReplayAmflowProxy$1 = {};

	Object.defineProperty(ReplayAmflowProxy$1, "__esModule", {
	  value: true
	});
	ReplayAmflowProxy$1.ReplayAmflowProxy = void 0;
	      ...
	      this._amflow.getTickList({
	        ...
	        if (!tickList) {
	          ...
	          if (!fromInGiven && !toInGiven) {
	            ...
	            // 前半重複
	            var ticksWithEvents = _this._sliceTicks(givenTicksWithEvents, from, to).concat(tickList[2 /* Ticks */] || []);
	            callback(null, [from, tickList[1 /* To */], ticksWithEvents]);
	          } else {
	            // 後半重複
	            var ticksWithEvents = (tickList[2 /* Ticks */] || []).concat(_this._sliceTicks(givenTicksWithEvents, from, to));
	...
	ReplayAmflowProxy$1.ReplayAmflowProxy = ReplayAmflowProxy;

	var SimpleProfiler$1 = {};
	...
	Object.defineProperty(AssetConfiguration, "__esModule", {
	  value: true
	});

	var GameConfiguration = {};

	Object.defineProperty(GameConfiguration, "__esModule", {
	  value: true
	});

	var OperationPluginInfo = {};

	Object.defineProperty(OperationPluginInfo, "__esModule", {
	  value: true
	});
	...
	(function (exports) {
	  ...
	  __exportStar(AssetConfiguration, exports);
	  __exportStar(GameConfiguration, exports);
	  __exportStar(OperationPluginInfo, exports);
	})(lib$1);

	var cjs = {};

	function isPromise(target) {
	  return target != null && (_typeof(target) === "object" || typeof target === "function") && typeof target.then === "function";
	}
	...
	var Trigger = /*#__PURE__*/function () {
	  ...
	  _createClass(Trigger, [{
	    key: "add",
	    value: function add(paramsOrFunc, owner) {
	      if (typeof paramsOrFunc === "function") {
	        ...
	        var index = typeof params.index === "number" ? params.index : undefined;
	        this._addHandler({
	          func: params.func,
	          owner: params.owner,
	          once: false,
	    ...
	    key: "addOnce",
	    value: function addOnce(paramsOrFunc, owner) {
	      if (typeof paramsOrFunc === "function") {
	        this._addHandler({
	          func: paramsOrFunc,
	          owner: owner,
	    ...
	    key: "handle",
	    value: function handle(owner, func, name) {
	      this.add(func ? {
	        owner: owner,
	        func: func,
	        name: name
	    ...
	    value: function fire(arg) {
	      ...
	      for (var i = 0; i < handlers.length; i++) {
	        ...
	        var returnedTruthy = !isPromise(ret) && !!ret;
	        if (returnedTruthy || handler.once) {
	          if (!this._handlers) continue;
	    ...
	    key: "contains",
	    value: function contains(paramsOrFunc, owner) {
	      var condition = typeof paramsOrFunc === "function" ? {
	        func: paramsOrFunc,
	        owner: owner
	      } : paramsOrFunc;
	    ...
	    key: "remove",
	    value: function remove(paramsOrFunc, owner) {
	      var condition = typeof paramsOrFunc === "function" ? {
	        func: paramsOrFunc,
	        owner: owner
	      } : paramsOrFunc;
	      for (var i = 0; i < this._handlers.length; i++) {
	        ...
	        if (condition.func === handler.func && condition.owner === handler.owner && condition.name === handler.name && condition.filter === handler.filter) {
	    ...
	    key: "removeAll",
	    value: function removeAll(params) {
	      var handlers = [];
	      if (params) {
	        for (var i = 0; i < this._handlers.length; i++) {
	          var handler = this._handlers[i];
	    ...
	    key: "_addHandler",
	    value: function _addHandler(params, index) {
	      if (index == null) {
	        this._handlers.push({
	          func: params.func,
	          owner: params.owner,
	    ...
	    key: "_comparePartial",
	    value: function _comparePartial(target, compare) {
	      ...
	      if (target.filter !== undefined && target.filter !== compare.filter) return false;
	      return true;
	    }
	  }]);
	  return Trigger;
	 ...
	 */
	var ChainTrigger = /*#__PURE__*/function (_Trigger) {
	  _inherits(ChainTrigger, _Trigger);
	  var _super = _createSuper(ChainTrigger);
	  /**
	  ...
	  function ChainTrigger(chain, filter, filterOwner) {
	    var _this;
	    _classCallCheck(this, ChainTrigger);
	  ...
	  _createClass(ChainTrigger, [{
	    key: "add",
	    value: function add(paramsOrHandler, owner) {
	      _get(_getPrototypeOf(ChainTrigger.prototype), "add", this).call(this, paramsOrHandler, owner);
	    ...
	    key: "addOnce",
	    value: function addOnce(paramsOrHandler, owner) {
	      _get(_getPrototypeOf(ChainTrigger.prototype), "addOnce", this).call(this, paramsOrHandler, owner);
	      if (!this._isActivated) {
	        this.chain.add(this._onChainTriggerFired, this);
	        this._isActivated = true;
	      }
	    ...
	    key: "remove",
	    value: function remove(paramsOrFunc, owner) {
	      _get(_getPrototypeOf(ChainTrigger.prototype), "remove", this).call(this, paramsOrFunc, owner);
	    ...
	    key: "removeAll",
	    value: function removeAll(params) {
	      _get(_getPrototypeOf(ChainTrigger.prototype), "removeAll", this).call(this, params);
	      if (this.length === 0 && this._isActivated) {
	        this.chain.remove(this._onChainTriggerFired, this);
	        this._isActivated = false;
	      }
	    ...
	    key: "destroy",
	    value: function destroy() {
	      _get(_getPrototypeOf(ChainTrigger.prototype), "destroy", this).call(this);
	      this.chain.remove(this._onChainTriggerFired, this);
	      this.filter = null;
	    ...
	    key: "_onChainTriggerFired",
	    value: function _onChainTriggerFired(args) {
	...
	Object.defineProperty(CompositeOperation$1, "__esModule", {
	  value: true
	});
	CompositeOperation$1.CompositeOperation = void 0;
	/**
	 * 描画時の合成方法。
	...
	})(CompositeOperation || (CompositeOperation$1.CompositeOperation = CompositeOperation = {}));

	var CompositeOperationString = {};

	Object.defineProperty(CompositeOperationString, "__esModule", {
	  value: true
	});

	var ImageData$1 = {};

	Object.defineProperty(ImageData$1, "__esModule", {
	  value: true
	});
	...
	Object.defineProperty(ShaderProgram$2, "__esModule", {
	  value: true
	});

	var ShaderUniform = {};

	Object.defineProperty(ShaderUniform, "__esModule", {
	  value: true
	});

	var Surface$1 = {};

	Object.defineProperty(Surface$1, "__esModule", {
	  value: true
	});

	var AudioAsset$3 = {};

	Object.defineProperty(AudioAsset$3, "__esModule", {
	  value: true
	});

	var AudioPlayer$3 = {};

	Object.defineProperty(AudioPlayer$3, "__esModule", {
	  value: true
	});

	var AudioSystem$2 = {};

	Object.defineProperty(AudioSystem$2, "__esModule", {
	  value: true
	});

	var AudioAssetHint = {};

	Object.defineProperty(AudioAssetHint, "__esModule", {
	  value: true
	});

	var ImageAssetHint = {};

	Object.defineProperty(ImageAssetHint, "__esModule", {
	  value: true
	});

	var ImageAsset = {};

	Object.defineProperty(ImageAsset, "__esModule", {
	  value: true
	});

	var ScriptAsset$2 = {};

	Object.defineProperty(ScriptAsset$2, "__esModule", {
	  value: true
	});

	var Module$2 = {};

	Object.defineProperty(Module$2, "__esModule", {
	  value: true
	});

	var ScriptAssetRuntimeValue = {};

	Object.defineProperty(ScriptAssetRuntimeValue, "__esModule", {
	  value: true
	});

	var TextAsset$2 = {};

	Object.defineProperty(TextAsset$2, "__esModule", {
	  value: true
	});

	var VideoPlayer = {};

	Object.defineProperty(VideoPlayer, "__esModule", {
	  value: true
	});

	var VideoSystem$2 = {};

	Object.defineProperty(VideoSystem$2, "__esModule", {
	  value: true
	});

	var VideoAsset = {};

	Object.defineProperty(VideoAsset, "__esModule", {
	  value: true
	});

	var VectorImageAsset = {};

	Object.defineProperty(VectorImageAsset, "__esModule", {
	  value: true
	});

	var VectorImageAssetHint = {};

	Object.defineProperty(VectorImageAssetHint, "__esModule", {
	  value: true
	});

	var BinaryAsset$1 = {};

	Object.defineProperty(BinaryAsset$1, "__esModule", {
	  value: true
	});

	var Asset$3 = {};

	Object.defineProperty(Asset$3, "__esModule", {
	  value: true
	});

	var AssetLoadErrorType$1 = {};

	Object.defineProperty(AssetLoadErrorType$1, "__esModule", {
	  value: true
	});
	AssetLoadErrorType$1.AssetLoadErrorType = void 0;
	/**
	 * アセット読み込み失敗時のエラーの種別。
	 *
	...
	})(AssetLoadErrorType || (AssetLoadErrorType$1.AssetLoadErrorType = AssetLoadErrorType = {}));

	var FontWeightString = {};

	Object.defineProperty(FontWeightString, "__esModule", {
	  value: true
	});

	var FontWeight$1 = {};

	Object.defineProperty(FontWeight$1, "__esModule", {
	  value: true
	});
	FontWeight$1.FontWeight = void 0;
	/**
	 * フォントのウェイト。
	...
	})(FontWeight || (FontWeight$1.FontWeight = FontWeight = {}));

	var FontFamily$1 = {};

	Object.defineProperty(FontFamily$1, "__esModule", {
	  value: true
	});
	FontFamily$1.FontFamily = void 0;
	/**
	 * 文字列描画のフォントファミリ。
	...
	})(FontFamily || (FontFamily$1.FontFamily = FontFamily = {}));

	var Glyph = {};

	Object.defineProperty(Glyph, "__esModule", {
	  value: true
	});

	var GlyphFactory$1 = {};

	Object.defineProperty(GlyphFactory$1, "__esModule", {
	  value: true
	});

	var Looper = {};

	Object.defineProperty(Looper, "__esModule", {
	  value: true
	});

	var OperationPluginView = {};

	Object.defineProperty(OperationPluginView, "__esModule", {
	  value: true
	});

	var OperationPluginViewInfo = {};

	Object.defineProperty(OperationPluginViewInfo, "__esModule", {
	  value: true
	});
	...
	Object.defineProperty(PlatformEventHandler, "__esModule", {
	  value: true
	});

	var PlatformPointEvent = {};

	Object.defineProperty(PlatformPointEvent, "__esModule", {
	  value: true
	});

	var RendererCandidate = {};

	Object.defineProperty(RendererCandidate, "__esModule", {
	  value: true
	});

	var RendererRequirement = {};

	Object.defineProperty(RendererRequirement, "__esModule", {
	  value: true
	});

	var ResourceFactory$1 = {};

	Object.defineProperty(ResourceFactory$1, "__esModule", {
	  value: true
	});
	...
	(function (exports) {
	  ...
	  __exportStar(CompositeOperation$1, exports);
	  __exportStar(CompositeOperationString, exports);
	  __exportStar(ImageData$1, exports);
	  __exportStar(Renderer, exports);
	  __exportStar(ShaderProgram$2, exports);
	  __exportStar(ShaderUniform, exports);
	  __exportStar(Surface$1, exports);
	  __exportStar(AudioAsset$3, exports);
	  __exportStar(AudioPlayer$3, exports);
	  __exportStar(AudioSystem$2, exports);
	  __exportStar(AudioAssetHint, exports);
	  __exportStar(ImageAssetHint, exports);
	  __exportStar(ImageAsset, exports);
	  __exportStar(ScriptAsset$2, exports);
	  __exportStar(Module$2, exports);
	  __exportStar(ScriptAssetRuntimeValue, exports);
	  __exportStar(TextAsset$2, exports);
	  __exportStar(VideoPlayer, exports);
	  __exportStar(VideoSystem$2, exports);
	  __exportStar(VideoAsset, exports);
	  __exportStar(VectorImageAsset, exports);
	  __exportStar(VectorImageAssetHint, exports);
	  __exportStar(BinaryAsset$1, exports);
	  __exportStar(Asset$3, exports);
	  __exportStar(AssetLoadErrorType$1, exports);
	  __exportStar(FontWeightString, exports);
	  __exportStar(FontWeight$1, exports);
	  __exportStar(FontFamily$1, exports);
	  __exportStar(Glyph, exports);
	  __exportStar(GlyphFactory$1, exports);
	  ...
	  __exportStar(RendererCandidate, exports);
	  __exportStar(RendererRequirement, exports);
	  __exportStar(ResourceFactory$1, exports);
	})(lib);

	var AudioSystem$1 = {};

	var AudioPlayContext$1 = {};

	Object.defineProperty(AudioPlayContext$1, "__esModule", {
	  value: true
	});
	AudioPlayContext$1.AudioPlayContext = void 0;
	var trigger_1$c = cjs;
	var AudioPlayContext = /** @class */function () {
	  function AudioPlayContext(param) {
	     ...
	     */
	    this.onPlay = new trigger_1$c.Trigger();
	    /**
	     * `stop()` が呼び出された時に通知される `Trigger` 。
	     */
	    this.onStop = new trigger_1$c.Trigger();
	...
	AudioPlayContext$1.AudioPlayContext = AudioPlayContext;

	var ExceptionFactory$2 = {};

	Object.defineProperty(ExceptionFactory$2, "__esModule", {
	  value: true
	});
	...
	(function (ExceptionFactory) {
	  ...
	  function createAssetLoadError(message, retriable, _type,
	  // 歴史的経緯により残っている値。利用していない。
	  cause) {
	    if (retriable === void 0) {
	      retriable = true;
	    }
	...
	})(ExceptionFactory$1 || (ExceptionFactory$2.ExceptionFactory = ExceptionFactory$1 = {}));

	var WeakRefKVS$1 = {};

	Object.defineProperty(WeakRefKVS$1, "__esModule", {
	  value: true
	});
	WeakRefKVS$1.WeakRefKVS = void 0;
	/**
	 * @private
	 */
	...
	WeakRefKVS$1.WeakRefKVS = WeakRefKVS;
	...
	Object.defineProperty(AudioSystem$1, "__esModule", {
	  value: true
	});
	AudioSystem$1.SoundAudioSystem = AudioSystem$1.MusicAudioSystem = AudioSystem$1.AudioSystem = void 0;
	var AudioPlayContext_1 = AudioPlayContext$1;
	var ExceptionFactory_1$g = ExceptionFactory$2;
	var WeakRefKVS_1$1 = WeakRefKVS$1;
	var AudioSystem = /** @class */function () {
	  function AudioSystem(param) {
	    ...
	    this._muted = false;
	    this._contextMap = new WeakRefKVS_1$1.WeakRefKVS();
	  ...
	  Object.defineProperty(AudioSystem.prototype, "volume", {
	    ...
	    set: function set(value) {
	      if (value < 0 || value > 1 || isNaN(value) || typeof value !== "number") throw ExceptionFactory_1$g.ExceptionFactory.createAssertionError("AudioSystem#volume: expected: 0.0-1.0, actual: " + value);
	      this._volume = value;
	  ...
	  AudioSystem.prototype._setPlaybackRate = function (value) {
	    if (value < 0 || isNaN(value) || typeof value !== "number") throw ExceptionFactory_1$g.ExceptionFactory.createAssertionError("AudioSystem#playbackRate: expected: greater or equal to 0.0, actual: " + value);
	...
	AudioSystem$1.AudioSystem = AudioSystem;
	var MusicAudioSystem = /** @class */function (_super) {
	  __extends$m(MusicAudioSystem, _super);
	  ...
	  MusicAudioSystem.prototype._handlePlay = function (e) {
	    if (e.player !== this._player) throw ExceptionFactory_1$g.ExceptionFactory.createAssertionError("MusicAudioSystem#_onPlayerPlayed: unexpected audio player");
	  };
	  /**
	   * @private
	   */
	...
	AudioSystem$1.MusicAudioSystem = MusicAudioSystem;
	var SoundAudioSystem = /** @class */function (_super) {
	  __extends$m(SoundAudioSystem, _super);
	...
	AudioSystem$1.SoundAudioSystem = SoundAudioSystem;

	var Module$1 = {};

	var PathUtil$3 = {};

	Object.defineProperty(PathUtil$3, "__esModule", {
	  value: true
	});
	PathUtil$3.PathUtil = void 0;
	/**
	 * パスユーティリティ。
	 */
	var PathUtil$2;
	    ...
	    for (var i = 0; i < parts.length; ++i) {
	      ...
	      switch (part) {
	        case "..":
	          ...
	          if (popped === undefined || popped === "" || popped === ".") throw new Error("PathUtil.resolvePath: invalid arguments");
	          break;
	        case ".":
	...
	})(PathUtil$2 || (PathUtil$3.PathUtil = PathUtil$2 = {}));

	Object.defineProperty(Module$1, "__esModule", {
	  value: true
	});
	Module$1.Module = void 0;
	var PathUtil_1$2 = PathUtil$3;
	/**
	...
	var Module = /** @class */function () {
	  function Module(param) {
	    var _this = this;
	    var path = param.path;
	    var dirname = PathUtil_1$2.PathUtil.resolveDirname(path);
	    // `virtualPath` と `virtualDirname` は　`DynamicAsset` の場合は `undefined` になる。
	    var virtualPath = param.virtualPath;
	    var virtualDirname = virtualPath ? PathUtil_1$2.PathUtil.resolveDirname(virtualPath) : undefined;
	    ...
	    this.loaded = false;
	    this.children = [];
	    this.paths = virtualDirname ? PathUtil_1$2.PathUtil.makeNodeModulePaths(virtualDirname) : [];
	...
	Object.defineProperty(ShaderProgram$1, "__esModule", {
	  value: true
	});
	ShaderProgram$1.ShaderProgram = void 0;
	/**
	...
	ShaderProgram$1.ShaderProgram = ShaderProgram;

	var VideoSystem$1 = {};

	Object.defineProperty(VideoSystem$1, "__esModule", {
	  value: true
	});
	VideoSystem$1.VideoSystem = void 0;
	/**
	...
	VideoSystem$1.VideoSystem = VideoSystem;
	...
	var PointDownEventBase = /** @class */function (_super) {
	  __extends$l(PointDownEventBase, _super);
	...
	var PointUpEventBase = /** @class */function (_super) {
	  __extends$l(PointUpEventBase, _super);
	...
	var PointMoveEventBase = /** @class */function (_super) {
	  __extends$l(PointMoveEventBase, _super);
	...
	Object.defineProperty(Object2D$1, "__esModule", {
	  value: true
	});
	Object2D$1.Object2D = void 0;
	var ExceptionFactory_1$f = ExceptionFactory$2;
	...
	var Object2D = /** @class */function () {
	  ...
	  Object2D.prototype.moveTo = function (posOrX, y) {
	    if (typeof posOrX === "number" && typeof y !== "number") {
	      throw ExceptionFactory_1$f.ExceptionFactory.createAssertionError("Object2D#moveTo: arguments must be CommonOffset or pair of x and y as a number.");
	    }
	  ...
	  Object2D.prototype.resizeTo = function (sizeOrWidth, height) {
	    if (typeof sizeOrWidth === "number" && typeof height !== "number") {
	      throw ExceptionFactory_1$f.ExceptionFactory.createAssertionError("Object2D#resizeTo: arguments must be CommonSize or pair of width and height as a number.");
	    }
	  ...
	  Object2D.prototype.getMatrix = function () {
	    if (!this._matrix) {
	      this._matrix = new Matrix_1$2.PlainMatrix();
	...
	(function (Util) {
	  ...
	  function enumToSnakeCase(enumDef, val) {
	    var s = enumDef[val];
	    // 呼び出し元で型が正しいことの保証が必要
	    return s[0].toLowerCase() + s.slice(1).replace(/[A-Z]/g, function (c) {
	  ...
	  Util.compositeOperationStringTable = (_a = {}, _a[pdi_types_1$1.CompositeOperation.SourceOver] = "source-over", _a[pdi_types_1$1.CompositeOperation.SourceAtop] = "source-atop", _a[pdi_types_1$1.CompositeOperation.Lighter] = "lighter", _a[pdi_types_1$1.CompositeOperation.Copy] = "copy", _a[pdi_types_1$1.CompositeOperation.ExperimentalSourceIn] = "experimental-source-in", _a[pdi_types_1$1.CompositeOperation.ExperimentalSourceOut] = "experimental-source-out", _a[pdi_types_1$1.CompositeOperation.ExperimentalDestinationAtop] = "experimental-destination-atop", _a[pdi_types_1$1.CompositeOperation.ExperimentalDestinationIn] = "experimental-destination-in", _a[pdi_types_1$1.CompositeOperation.DestinationOut] = "destination-out", _a[pdi_types_1$1.CompositeOperation.DestinationOver] = "destination-over", _a[pdi_types_1$1.CompositeOperation.Xor] = "xor", _a[pdi_types_1$1.CompositeOperation.Difference] = "difference", _a[pdi_types_1$1.CompositeOperation.Saturation] = "saturation", _a);
	})(Util || (Util$1.Util = Util = {}));
	...
	Object.defineProperty(E$1, "__esModule", {
	  value: true
	});
	E$1.E = E$1.PointMoveEvent = E$1.PointUpEvent = E$1.PointDownEvent = void 0;
	var trigger_1$b = cjs;
	var Event_1$1 = Event;
	var ExceptionFactory_1$e = ExceptionFactory$2;
	var Matrix_1$1 = Matrix;
	var Object2D_1$2 = Object2D$1;
	var Util_1$5 = Util$1;
	/**
	 * ポインティング操作の開始を表すイベント。
	 */
	var PointDownEvent = /** @class */function (_super) {
	  __extends$k(PointDownEvent, _super);
	  function PointDownEvent() {
	    return _super !== null && _super.apply(this, arguments) || this;
	  }
	  return PointDownEvent;
	}(Event_1$1.PointDownEventBase);
	E$1.PointDownEvent = PointDownEvent;
	/**
	 * ポインティング操作の終了を表すイベント。
	 * PointDownEvent後にのみ発生する。
	 *
	...
	var PointUpEvent = /** @class */function (_super) {
	  __extends$k(PointUpEvent, _super);
	  function PointUpEvent() {
	    return _super !== null && _super.apply(this, arguments) || this;
	  }
	  return PointUpEvent;
	}(Event_1$1.PointUpEventBase);
	E$1.PointUpEvent = PointUpEvent;
	/**
	 * ポインティング操作の移動を表すイベント。
	 * PointDownEvent後にのみ発生するため、MouseMove相当のものが本イベントとして発生することはない。
	 *
	...
	var E = /** @class */function (_super) {
	  ...
	  function E(param) {
	    ...
	    if (param.children) {
	      for (var i = 0; i < param.children.length; ++i) _this.append(param.children[i]);
	    }
	    if (param.parent) {
	      param.parent.append(_this);
	    }
	    if (param.touchable != null) _this.touchable = param.touchable;
	    if (!!param.hidden) _this.hide();
	    // set id, scene
	  ...
	  Object.defineProperty(E.prototype, "onUpdate", {
	    ...
	    get: function get() {
	      if (!this._onUpdate) this._onUpdate = new trigger_1$b.ChainTrigger(this.scene.onUpdate);
	  ...
	  Object.defineProperty(E.prototype, "onMessage", {
	    ...
	    // Eの生成コスト低減を考慮し、参照された時のみ生成出来るようアクセサを使う
	    get: function get() {
	      if (!this._onMessage) this._onMessage = new trigger_1$b.ChainTrigger(this.scene.onMessage);
	  ...
	  Object.defineProperty(E.prototype, "onPointDown", {
	    ...
	    // Eの生成コスト低減を考慮し、参照された時のみ生成出来るようアクセサを使う
	    get: function get() {
	      if (!this._onPointDown) this._onPointDown = new trigger_1$b.ChainTrigger(this.scene.onPointDownCapture, this._isTargetOperation, this);
	  ...
	  Object.defineProperty(E.prototype, "onPointUp", {
	    ...
	    // Eの生成コスト低減を考慮し、参照された時のみ生成出来るようアクセサを使う
	    get: function get() {
	      if (!this._onPointUp) this._onPointUp = new trigger_1$b.ChainTrigger(this.scene.onPointUpCapture, this._isTargetOperation, this);
	  ...
	  Object.defineProperty(E.prototype, "onPointMove", {
	    ...
	    // Eの生成コスト低減を考慮し、参照された時のみ生成出来るようアクセサを使う
	    get: function get() {
	      if (!this._onPointMove) this._onPointMove = new trigger_1$b.ChainTrigger(this.scene.onPointMoveCapture, this._isTargetOperation, this);
	  ...
	  E.prototype.render = function (renderer, camera) {
	    ...
	    if (this.state & 1 /* EntityStateFlags.Hidden */) return;
	    ...
	    if (op !== undefined) {
	      renderer.setCompositeOperation(typeof op === "string" ? op : Util_1$5.Util.compositeOperationStringTable[op]);
	    }
	  ...
	  E.prototype.remove = function (e) {
	    ...
	    if (index < 0) throw ExceptionFactory_1$e.ExceptionFactory.createAssertionError("E#remove: invalid child");
	  ...
	  E.prototype.destroy = function () {
	    ...
	    if (this.children) {
	      ...
	      if (this.children.length !== 0) throw ExceptionFactory_1$e.ExceptionFactory.createAssertionError("E#destroy: can not destroy all children, " + this.children.length);
	  ...
	  E.prototype.modified = function (_isBubbling) {
	    ...
	    if (this.angle || this.scaleX !== 1 || this.scaleY !== 1 || this.anchorX !== 0 || this.anchorY !== 0 || this.opacity !== 1 || this.compositeOperation !== undefined || this.shaderProgram !== undefined) {
	      this.state &= ~8 /* EntityStateFlags.ContextLess */;
	    } else {
	      this.state |= 8 /* EntityStateFlags.ContextLess */;
	    }

	    if (this.state & 4 /* EntityStateFlags.Modified */) return;
	    this.state |= 4 /* EntityStateFlags.Modified */;
	  ...
	  E.prototype.findPointSourceByPoint = function (point, m, force) {
	    if (this.state & 1 /* EntityStateFlags.Hidden */) return undefined;
	  ...
	  E.prototype.globalToLocal = function (offset) {
	    var matrix = new Matrix_1$1.PlainMatrix();
	    for (var entity = this; entity instanceof E; entity = entity.parent) {
	  ...
	  E.prototype._isTargetOperation = function (e) {
	    if (this.state & 1 /* EntityStateFlags.Hidden */) return false;
	    if (e instanceof Event_1$1.PointEventBase) return this._touchable && e.target === this;
	    return false;
	  };
	...
	Object.defineProperty(CacheableE$1, "__esModule", {
	  value: true
	});
	CacheableE$1.CacheableE = void 0;
	var E_1$4 = E$1;
	/**
	 * 内部描画キャッシュを持つ `E` 。
	 */
	var CacheableE = /** @class */function (_super) {
	  __extends$j(CacheableE, _super);
	  /**
	...
	CacheableE$1.CacheableE = CacheableE;
	...
	Object.defineProperty(FilledRect$1, "__esModule", {
	  value: true
	});
	FilledRect$1.FilledRect = void 0;
	var ExceptionFactory_1$d = ExceptionFactory$2;
	var E_1$3 = E$1;
	/**
	 * 塗りつぶされた矩形を表すエンティティ。
	 */
	var FilledRect = /** @class */function (_super) {
	  __extends$i(FilledRect, _super);
	  /**
	  ...
	  function FilledRect(param) {
	    var _this = _super.call(this, param) || this;
	    if (typeof param.cssColor !== "string") throw ExceptionFactory_1$d.ExceptionFactory.createTypeMismatchError("ColorBox#constructor(cssColor)", "string", param.cssColor);
	...
	FilledRect$1.FilledRect = FilledRect;
	...
	Object.defineProperty(SurfaceUtil$1, "__esModule", {
	  value: true
	});
	SurfaceUtil$1.SurfaceUtil = void 0;
	var ExceptionFactory_1$c = ExceptionFactory$2;
	/**
	...
	(function (SurfaceUtil) {
	  ...
	  function asSurface(src) {
	    ...
	    throw ExceptionFactory_1$c.ExceptionFactory.createTypeMismatchError("SurfaceUtil#asSurface", "ImageAsset|Surface", src);
	  }
	...
	})(SurfaceUtil || (SurfaceUtil$1.SurfaceUtil = SurfaceUtil = {}));
	...
	Object.defineProperty(Sprite$1, "__esModule", {
	  value: true
	});
	Sprite$1.Sprite = void 0;
	var Matrix_1 = Matrix;
	var SurfaceUtil_1$3 = SurfaceUtil$1;
	var E_1$2 = E$1;
	/**
	 * 画像を描画するエンティティ。
	 */
	var Sprite = /** @class */function (_super) {
	  __extends$h(Sprite, _super);
	  /**
	  ...
	  function Sprite(param) {
	    ...
	    if ("_drawable" in param.src) {
	      ...
	      // @ts-ignore
	      _this._surface = SurfaceUtil_1$3.SurfaceUtil.asSurface(param.src);
	    }
	    if (param.width == null) _this.width = _this._surface.width;
	    if (param.height == null) _this.height = _this._surface.height;
	    ...
	    SurfaceUtil_1$3.SurfaceUtil.setupAnimatingHandler(_this, _this._surface);
	  ...
	  Sprite.prototype._invalidateSelf = function () {
	    ...
	    if (this.src !== this._beforeSrc) {
	      ...
	      if ("_drawable" in this.src) {
	        ...
	        // @ts-ignore
	        this._surface = SurfaceUtil_1$3.SurfaceUtil.asSurface(this.src);
	      }
	    }
	    if (this._surface !== this._beforeSurface) {
	      SurfaceUtil_1$3.SurfaceUtil.migrateAnimatingHandler(this, this._beforeSurface, this._surface);
	...
	Object.defineProperty(FrameSprite$1, "__esModule", {
	  value: true
	});
	FrameSprite$1.FrameSprite = void 0;
	var trigger_1$a = cjs;
	var Sprite_1$1 = Sprite$1;
	/**
	...
	var FrameSprite = /** @class */function (_super) {
	  __extends$g(FrameSprite, _super);
	  /**
	  ...
	  function FrameSprite(param) {
	    ...
	    _this.onFinish = new trigger_1$a.Trigger();
	...
	FrameSprite$1.FrameSprite = FrameSprite;

	var Label$1 = {};

	var TextAlign$1 = {};

	Object.defineProperty(TextAlign$1, "__esModule", {
	  value: true
	});
	TextAlign$1.TextAlign = void 0;
	/**
	 * テキストの描画位置。
	...
	})(TextAlign || (TextAlign$1.TextAlign = TextAlign = {}));
	...
	Object.defineProperty(Label$1, "__esModule", {
	  value: true
	});
	Label$1.Label = void 0;
	var TextAlign_1 = TextAlign$1;
	var Util_1$4 = Util$1;
	var CacheableE_1$1 = CacheableE$1;
	/**
	 * 単一行のテキストを描画するエンティティ。
	 * 本クラスの利用には `BitmapFont` または `DynamicFont` が必要となる。
	 */
	var Label = /** @class */function (_super) {
	  __extends$f(Label, _super);
	  /**
	  ...
	  Label.prototype.renderSelfFromCache = function (renderer) {
	    ...
	    renderer.drawImage(this._cache, 0, 0, this._cacheSize.width + CacheableE_1$1.CacheableE.PADDING, this._cacheSize.height + CacheableE_1$1.CacheableE.PADDING, destOffsetX, 0);
	  };
	  ...
	  Label.prototype._invalidateSelf = function () {
	    ...
	    this._realTextAlign = typeof align === "string" ? align : Util_1$4.Util.enumToSnakeCase(TextAlign_1.TextAlign, align);
	    if (!this.fontSize) {
	      this.height = 0;
	      return;
	    }
	    ...
	    for (var i = this.text.length - 1; i >= 0; --i) {
	      var code = Util_1$4.Util.charCodeAt(this.text, i);
	      if (!code) {
	        continue;
	      }
	  ...
	  Label.prototype._outputOfWarnLogWithNoGlyph = function (code, functionName) {
	    ...
	    console.warn("Label#" + functionName + ": failed to get a glyph for '" + str + "' " + "(BitmapFont might not have the glyph or DynamicFont might create a glyph larger than its atlas).");
	  };
	  return Label;
	}(CacheableE_1$1.CacheableE);
	Label$1.Label = Label;
	...
	Object.defineProperty(Pane$1, "__esModule", {
	  value: true
	});
	Pane$1.Pane = void 0;
	var SurfaceUtil_1$2 = SurfaceUtil$1;
	var CacheableE_1 = CacheableE$1;
	/**
	 * 枠を表すエンティティ。
	...
	var Pane = /** @class */function (_super) {
	  ...
	  function Pane(param) {
	    ...
	    _this._backgroundImageSurface = SurfaceUtil_1$2.SurfaceUtil.asSurface(param.backgroundImage);
	  ...
	  Pane.prototype.modified = function (isBubbling) {
	    if (isBubbling) this.state &= ~2 /* EntityStateFlags.Cached */;
	  ...
	  Pane.prototype._renderBackground = function () {
	    if (this.backgroundImage !== this._beforeBackgroundImage) {
	      this._backgroundImageSurface = SurfaceUtil_1$2.SurfaceUtil.asSurface(this.backgroundImage);
	...
	Object.defineProperty(AssetAccessor$1, "__esModule", {
	  value: true
	});
	AssetAccessor$1.AssetAccessor = void 0;
	/**
	...
	AssetAccessor$1.AssetAccessor = AssetAccessor;

	var AssetGenerationConfiguration = {};

	Object.defineProperty(AssetGenerationConfiguration, "__esModule", {
	  value: true
	});

	var AssetHolder$1 = {};

	Object.defineProperty(AssetHolder$1, "__esModule", {
	  value: true
	});
	AssetHolder$1.AssetHolder = void 0;
	var ExceptionFactory_1$b = ExceptionFactory$2;
	/**
	...
	var AssetHolder = /** @class */function () {
	  ...
	  AssetHolder.prototype._decrementWaitingAssetCount = function () {
	    ...
	    if (this.waitingAssetsCount < 0) throw ExceptionFactory_1$b.ExceptionFactory.createAssertionError("AssetHolder#_onAssetLoad: broken waitingAssetsCount");
	    var hs = this._handlerSet;
	  ...
	  AssetHolder.prototype._peekAssetConfFromAssetId = function (id) {
	    ...
	    throw ExceptionFactory_1$b.ExceptionFactory.createAssertionError("AssetHolder#_peekAssetConfFromAssetId: could not peek the asset: ".concat(id));
	  };
	  return AssetHolder;
	}();
	AssetHolder$1.AssetHolder = AssetHolder;

	var AssetLoadFailureInfo = {};

	Object.defineProperty(AssetLoadFailureInfo, "__esModule", {
	  value: true
	});

	var AssetManager$1 = {};

	var EmptyBinaryAsset$1 = {};

	Object.defineProperty(EmptyBinaryAsset$1, "__esModule", {
	  value: true
	});
	EmptyBinaryAsset$1.EmptyBinaryAsset = void 0;
	var trigger_1$9 = cjs;
	var EmptyBinaryAsset = /** @class */function () {
	  function EmptyBinaryAsset(id, path) {
	    this.type = "binary";
	    this.onDestroyed = new trigger_1$9.Trigger();
	    this.id = id;
	    this.path = path;
	...
	EmptyBinaryAsset$1.EmptyBinaryAsset = EmptyBinaryAsset;

	var EmptyGeneratedVectorImageAsset$1 = {};

	var EmptyVectorImageAsset$1 = {};

	Object.defineProperty(EmptyVectorImageAsset$1, "__esModule", {
	  value: true
	});
	EmptyVectorImageAsset$1.EmptyVectorImageAsset = void 0;
	var trigger_1$8 = cjs;
	var EmptyVectorImageAsset = /** @class */function () {
	  function EmptyVectorImageAsset(id, path, width, height, hint) {
	    ...
	    this.width = 0;
	    this.height = 0;
	    this.onDestroyed = new trigger_1$8.Trigger();
	    this.id = id;
	    this.path = path;
	    this.originalPath = path;
	    this.width = width;
	...
	EmptyVectorImageAsset$1.EmptyVectorImageAsset = EmptyVectorImageAsset;
	...
	Object.defineProperty(EmptyGeneratedVectorImageAsset$1, "__esModule", {
	  value: true
	});
	EmptyGeneratedVectorImageAsset$1.EmptyGeneratedVectorImageAsset = void 0;
	var EmptyVectorImageAsset_1$1 = EmptyVectorImageAsset$1;
	var EmptyGeneratedVectorImageAsset = /** @class */function (_super) {
	  __extends$d(EmptyGeneratedVectorImageAsset, _super);
	...
	}(EmptyVectorImageAsset_1$1.EmptyVectorImageAsset);
	EmptyGeneratedVectorImageAsset$1.EmptyGeneratedVectorImageAsset = EmptyGeneratedVectorImageAsset;

	var PartialImageAsset$1 = {};

	Object.defineProperty(PartialImageAsset$1, "__esModule", {
	  value: true
	});
	PartialImageAsset$1.PartialImageAsset = void 0;
	var trigger_1$7 = cjs;
	/**
	 * 部分画像アセット。
	 *
	...
	var PartialImageAsset = /** @class */function () {
	  ...
	  function PartialImageAsset(resourceFactory, id, uri, width, height, slice) {
	    this.type = "image";
	    this.hint = undefined;
	    this.onDestroyed = new trigger_1$7.Trigger();
	    this._surface = null;
	...
	PartialImageAsset$1.PartialImageAsset = PartialImageAsset;

	Object.defineProperty(AssetManager$1, "__esModule", {
	  value: true
	});
	AssetManager$1.AssetManager = void 0;
	var EmptyBinaryAsset_1 = EmptyBinaryAsset$1;
	var EmptyGeneratedVectorImageAsset_1 = EmptyGeneratedVectorImageAsset$1;
	var EmptyVectorImageAsset_1 = EmptyVectorImageAsset$1;
	var PartialImageAsset_1 = PartialImageAsset$1;
	var ExceptionFactory_1$a = ExceptionFactory$2;
	var VideoSystem_1 = VideoSystem$1;
	/**
	 * @ignore
	 */
	...
	var AssetManager = /** @class */function () {
	  ...
	  AssetManager.prototype.retryLoad = function (asset) {
	    if (!this._loadings.hasOwnProperty(asset.id)) throw ExceptionFactory_1$a.ExceptionFactory.createAssertionError("AssetManager#retryLoad: invalid argument.");
	    ...
	    if (loadingInfo.errorCount > AssetManager.MAX_ERROR_COUNT) {
	      ...
	      throw ExceptionFactory_1$a.ExceptionFactory.createAssertionError("AssetManager#retryLoad: too many retrying.");
	    }
	  ...
	  AssetManager.prototype.preloadScriptAssetIds = function () {
	    return Object.entries(this.configuration).filter(function (_a) {
	      var conf = _a[1];
	  ...
	  AssetManager.prototype.peekLiveAssetByAccessorPath = function (accessorPath, type) {
	    ...
	    if (accessorPath[0] !== "/") throw ExceptionFactory_1$a.ExceptionFactory.createAssertionError("AssetManager#peekLiveAssetByAccessorPath(): accessorPath must start with '/'");
	    var vpath = accessorPath.slice(1); // accessorPath から "/" を削ると virtualPath という仕様
	    var asset = this._liveAssetVirtualPathTable[vpath];
	    if (!asset || type !== asset.type) throw ExceptionFactory_1$a.ExceptionFactory.createAssertionError("AssetManager#peekLiveAssetByAccessorPath(): No ".concat(type, " asset for ").concat(accessorPath));
	  ...
	  AssetManager.prototype.peekLiveAssetById = function (assetId, type) {
	    ...
	    if (!asset || type !== asset.type) throw ExceptionFactory_1$a.ExceptionFactory.createAssertionError("AssetManager#peekLiveAssetById(): No ".concat(type, " asset for id ").concat(assetId));
	    return asset; // asset.typeを直前で確認しているので確実にTになるが、型推論できないのでキャストする
	  };
	  /**
	  ...
	  AssetManager.prototype._normalize = function (configuration) {
	    var ret = {};
	    if (!(configuration instanceof Object)) throw ExceptionFactory_1$a.ExceptionFactory.createAssertionError("AssetManager#_normalize: invalid arguments.");
	    for (var p in configuration) {
	      ...
	      if (!conf.path) {
	        throw ExceptionFactory_1$a.ExceptionFactory.createAssertionError("AssetManager#_normalize: No path given for: " + p);
	      }
	      if (!conf.virtualPath) {
	        throw ExceptionFactory_1$a.ExceptionFactory.createAssertionError("AssetManager#_normalize: No virtualPath given for: " + p);
	      }
	  ...
	  AssetManager.prototype._normalizeAssetBaseDeclaration = function (assetId, conf) {
	    if (!conf.type) {
	      throw ExceptionFactory_1$a.ExceptionFactory.createAssertionError("AssetManager#_normalize: No type given for: " + assetId);
	    }
	    if (conf.type === "image") {
	      if (typeof conf.width !== "number") throw ExceptionFactory_1$a.ExceptionFactory.createAssertionError("AssetManager#_normalize: wrong width given for the image asset: " + assetId);
	      if (typeof conf.height !== "number") throw ExceptionFactory_1$a.ExceptionFactory.createAssertionError("AssetManager#_normalize: wrong height given for the image asset: " + assetId);
	    ...
	    if (conf.type === "audio") {
	      ...
	      if (conf.systemId !== "music" && conf.systemId !== "sound") {
	        throw ExceptionFactory_1$a.ExceptionFactory.createAssertionError("AssetManager#_normalize: wrong systemId given for the audio asset: " + assetId);
	      }
	    }
	    if (conf.type === "video") {
	      if (!conf.useRealSize) {
	        if (typeof conf.width !== "number") throw ExceptionFactory_1$a.ExceptionFactory.createAssertionError("AssetManager#_normalize: wrong width given for the video asset: " + assetId);
	        if (typeof conf.height !== "number") throw ExceptionFactory_1$a.ExceptionFactory.createAssertionError("AssetManager#_normalize: wrong height given for the video asset: " + assetId);
	        conf.useRealSize = false;
	      }
	    }
	    if (conf.type === "vector-image") {
	      if (typeof conf.width !== "number") throw ExceptionFactory_1$a.ExceptionFactory.createAssertionError("AssetManager#_normalize: wrong width given for the vector-image asset: " + assetId);
	      if (typeof conf.height !== "number") throw ExceptionFactory_1$a.ExceptionFactory.createAssertionError("AssetManager#_normalize: wrong height given for the vector-image asset: " + assetId);
	    }
	    return conf;
	  };
	  /**
	  ...
	  AssetManager.prototype._createAssetFor = function (idOrConf) {
	    ...
	    if (!conf) throw ExceptionFactory_1$a.ExceptionFactory.createAssertionError("AssetManager#_createAssetFor: unknown asset ID: " + id);
	    var type = conf.type;
	    switch (type) {
	      case "image":
	        ...
	        var asset = conf.slice ? new PartialImageAsset_1.PartialImageAsset(resourceFactory, id, uri, conf.width, conf.height, conf.slice) : resourceFactory.createImageAsset(id, uri, conf.width, conf.height);
	      ...
	      default:
	        throw ExceptionFactory_1$a.ExceptionFactory.createAssertionError("AssertionError#_createAssetFor: unknown asset type " + type + " for asset ID: " + id);
	    }
	  };
	  /**
	   * @private
	  ...
	  AssetManager.prototype._createGeneratedAssetFor = function (conf) {
	    ...
	    switch (conf.type) {
	      ...
	      default:
	        throw ExceptionFactory_1$a.ExceptionFactory.createAssertionError("AssertionError#_createFromAssetGenerationFor: unsupported asset type ".concat(conf.type, " for asset ID: ").concat(conf.id));
	    }
	  };
	  /**
	   * @ignore
	  ...
	  AssetManager.prototype._releaseAsset = function (assetId) {
	    ...
	    if (asset) {
	      ...
	      if (asset.inUse()) {
	        if (asset.type === "audio") {
	          ...
	          throw ExceptionFactory_1$a.ExceptionFactory.createAssertionError("AssetManager#unrefAssets: Unsupported in-use " + asset.id);
	        }
	      } else {
	        asset.destroy();
	      }
	    ...
	    if (this.configuration[assetId]) {
	      ...
	      if (virtualPath && this._liveAssetVirtualPathTable.hasOwnProperty(virtualPath)) delete this._liveAssetVirtualPathTable[virtualPath];
	  ...
	  AssetManager.prototype._onAssetError = function (asset, error) {
	    ...
	    if (loadingInfo.errorCount > AssetManager.MAX_ERROR_COUNT && error.retriable) {
	      error = ExceptionFactory_1$a.ExceptionFactory.createAssetLoadError("Retry limit exceeded", false, null, error);
	    }
	  ...
	  AssetManager.prototype._replaceModulePathToAbsolute = function (accessorPath) {
	    if (accessorPath[0] === "/" || accessorPath[0] === "*" // パスに `**/*` が指定された場合
	    ) {
	      return accessorPath;
	    }
	  ...
	  AssetManager.prototype._addAssetToTables = function (asset) {
	    ...
	    if (this.configuration[asset.id]) {
	      ...
	      if (!this._liveAssetVirtualPathTable.hasOwnProperty(virtualPath)) {
	        ...
	        if (this._liveAssetVirtualPathTable[virtualPath].path !== asset.path) throw ExceptionFactory_1$a.ExceptionFactory.createAssertionError("AssetManager#_onAssetLoad(): duplicated asset path");
	      }
	...
	AssetManager$1.AssetManager = AssetManager;

	var AssetManagerLoadHandler = {};

	Object.defineProperty(AssetManagerLoadHandler, "__esModule", {
	  value: true
	});

	var AudioSystemManager$1 = {};

	Object.defineProperty(AudioSystemManager$1, "__esModule", {
	  value: true
	});
	AudioSystemManager$1.AudioSystemManager = void 0;
	var AudioSystem_1 = AudioSystem$1;
	var ExceptionFactory_1$9 = ExceptionFactory$2;
	/**
	 * `AudioSystem` の管理クラス。
	 *
	...
	var AudioSystemManager = /** @class */function () {
	  ...
	  AudioSystemManager.prototype.create = function (asset) {
	    if (asset._system.id === "music") {
	      ...
	      throw ExceptionFactory_1$9.ExceptionFactory.createAssertionError("AudioSystemManager#create(): unknown systemId \"".concat(asset._system.id, "\" for asset ID \"").concat(asset.id, "\""));
	    }
	  };
	  /**
	  ...
	  AudioSystemManager.prototype.play = function (asset) {
	    if (asset._system.id === "music") {
	      ...
	      throw ExceptionFactory_1$9.ExceptionFactory.createAssertionError("AudioSystemManager#play(): unknown systemId \"".concat(asset._system.id, "\" for asset ID \"").concat(asset.id, "\""));
	    }
	  };
	  /**
	   * @private
	...
	AudioSystemManager$1.AudioSystemManager = AudioSystemManager;

	var AudioUtil$1 = {};

	Object.defineProperty(AudioUtil$1, "__esModule", {
	  value: true
	});
	AudioUtil$1.AudioUtil = void 0;
	var Util_1$3 = Util$1;
	/**
	 * linear のイージング関数。
	 */
	...
	(function (AudioUtil) {
	  ...
	  function fadeIn(game, context, duration, to, easing) {
	    ...
	    context.play();
	    var _a = transitionVolume(game, context, duration, to, easing),
	  ...
	  function fadeOut(game, context, duration, easing) {
	    ...
	    var _a = transitionVolume(game, context, duration, 0, easing),
	  ...
	  function transitionVolume(game, context, duration, to, easing) {
	    ...
	    var elapsed = 0;
	    context.changeVolume(Util_1$3.Util.clamp(from, 0, 1));
	    var handler = function handler() {
	      elapsed += frame;
	      if (elapsed <= duration) {
	        var progress = easing(elapsed, from, to - from, duration);
	        context.changeVolume(Util_1$3.Util.clamp(progress, 0, 1));
	        return false;
	      } else {
	...
	})(AudioUtil || (AudioUtil$1.AudioUtil = AudioUtil = {}));

	var BitmapFont$1 = {};

	var Font$1 = {};

	Object.defineProperty(Font$1, "__esModule", {
	  value: true
	});
	Font$1.Font = void 0;
	var Util_1$2 = Util$1;
	...
	var Font = /** @class */function () {
	  ...
	  Font.prototype.measureText = function (text) {
	    ...
	    for (var i = 0; i < text.length; i++) {
	      var code = Util_1$2.Util.charCodeAt(text, i);
	      if (!code) continue;
	...
	Object.defineProperty(BitmapFont$1, "__esModule", {
	  value: true
	});
	BitmapFont$1.BitmapFont = void 0;
	var Font_1$1 = Font$1;
	var SurfaceUtil_1$1 = SurfaceUtil$1;
	/**
	 * ラスタ画像によるフォント。
	 */
	var BitmapFont = /** @class */function (_super) {
	  __extends$c(BitmapFont, _super);
	  /**
	  ...
	  function BitmapFont(param) {
	    ...
	    // @ts-ignore
	    _this.surface = SurfaceUtil_1$1.SurfaceUtil.asSurface(param.src);
	    if (param.glyphInfo) {
	...
	BitmapFont$1.BitmapFont = BitmapFont;

	var Camera = {};

	Object.defineProperty(Camera, "__esModule", {
	  value: true
	});
	...
	Object.defineProperty(Camera2D$1, "__esModule", {
	  value: true
	});
	Camera2D$1.Camera2D = void 0;
	var Object2D_1$1 = Object2D$1;
	/**
	 * 2D世界におけるカメラ。
	 */
	var Camera2D = /** @class */function (_super) {
	  __extends$b(Camera2D, _super);
	  /**
	  ...
	  return Camera2D;
	}(Object2D_1$1.Object2D);
	Camera2D$1.Camera2D = Camera2D;

	var Collision$1 = {};

	Object.defineProperty(Collision$1, "__esModule", {
	  value: true
	});
	Collision$1.Collision = void 0;
	var Util_1$1 = Util$1;
	// 外積の絶対値
	...
	(function (Collision) {
	  ...
	  function intersectEntities(e1, e2, area1, area2) {
	    ...
	    var r1 = area1 ? {
	      left: area1.x,
	      top: area1.y,
	      right: area1.x + area1.width,
	      bottom: area1.y + area1.height
	    } : {
	      left: 0,
	      top: 0,
	      right: e1.width,
	    ...
	    var r2 = area2 ? {
	      left: area2.x,
	      top: area2.y,
	      right: area2.x + area2.width,
	      bottom: area2.y + area2.height
	    } : {
	      left: 0,
	      top: 0,
	      right: e2.width,
	    ...
	    var lt1 = mat1.multiplyPoint({
	      x: r1.left,
	      y: r1.top
	    });
	    var rt1 = mat1.multiplyPoint({
	      x: r1.right,
	      y: r1.top
	    });
	    var lb1 = mat1.multiplyPoint({
	      x: r1.left,
	      y: r1.bottom
	    });
	    var rb1 = mat1.multiplyPoint({
	      x: r1.right,
	      y: r1.bottom
	    });
	    var lt2 = mat2.multiplyPoint({
	      x: r2.left,
	      y: r2.top
	    });
	    var rt2 = mat2.multiplyPoint({
	      x: r2.right,
	      y: r2.top
	    });
	    var lb2 = mat2.multiplyPoint({
	      x: r2.left,
	      y: r2.bottom
	    });
	    var rb2 = mat2.multiplyPoint({
	      x: r2.right,
	      y: r2.bottom
	    });
	    ...
	    if (Collision.intersectLineSegments(lt1, rt1, lt2, rt2) || Collision.intersectLineSegments(lt1, rt1, rt2, rb2) || Collision.intersectLineSegments(lt1, rt1, rb2, lb2) || Collision.intersectLineSegments(lt1, rt1, lb2, lt2) || Collision.intersectLineSegments(rt1, rb1, lt2, rt2) || Collision.intersectLineSegments(rt1, rb1, rt2, rb2) || Collision.intersectLineSegments(rt1, rb1, rb2, lb2) || Collision.intersectLineSegments(rt1, rb1, lb2, lt2) || Collision.intersectLineSegments(rb1, lb1, lt2, rt2) || Collision.intersectLineSegments(rb1, lb1, rt2, rb2) || Collision.intersectLineSegments(rb1, lb1, rb2, lb2) || Collision.intersectLineSegments(rb1, lb1, lb2, lt2) || Collision.intersectLineSegments(lb1, lt1, lt2, rt2) || Collision.intersectLineSegments(lb1, lt1, rt2, rb2) || Collision.intersectLineSegments(lb1, lt1, rb2, lb2) || Collision.intersectLineSegments(lb1, lt1, lb2, lt2)) {
	      return true;
	    }
	    ...
	    if (s1 * absCross(sub(lb1, lt1), sub(lt2, lt1)) >= 0 && s1 * absCross(sub(rb1, lb1), sub(lt2, lb1)) >= 0 && s1 * absCross(sub(rt1, rb1), sub(lt2, rb1)) >= 0) {
	      return true;
	    }
	    // そうでない場合、e2 が e1 を包含しているなら衝突している。
	    var s2 = absCross(sub(lt2, rt2), sub(lt1, rt2));
	    return s2 * absCross(sub(lb2, lt2), sub(lt1, lt2)) >= 0 && s2 * absCross(sub(rb2, lb2), sub(lt1, lb2)) >= 0 && s2 * absCross(sub(rt2, rb2), sub(lt1, rb2)) >= 0;
	  }
	  ...
	  function within(t1x, t1y, t2x, t2y, distance) {
	    ...
	    return distance >= Util_1$1.Util.distance(t1x, t1y, t2x, t2y);
	  }
	  ...
	  function withinAreas(t1, t2, distance) {
	    ...
	    return distance >= Util_1$1.Util.distanceBetweenAreas(t1, t2);
	  }
	  Collision.withinAreas = withinAreas;
	})(Collision || (Collision$1.Collision = Collision = {}));

	var DefaultLoadingScene$1 = {};

	var CameraCancellingE$1 = {};
	...
	Object.defineProperty(CameraCancellingE$1, "__esModule", {
	  value: true
	});
	CameraCancellingE$1.CameraCancellingE = void 0;
	var Camera2D_1$1 = Camera2D$1;
	var Object2D_1 = Object2D$1;
	var E_1$1 = E$1;
	/**
	...
	var CameraCancellingE = /** @class */function (_super) {
	  __extends$a(CameraCancellingE, _super);
	  ...
	  CameraCancellingE.prototype.renderSelf = function (renderer, camera) {
	    ...
	    if (camera && camera instanceof Camera2D_1$1.Camera2D) {
	      var c = camera;
	      var canceller = this._canceller;
	      if (c.x !== canceller.x || c.y !== canceller.y || c.angle !== canceller.angle || c.scaleX !== canceller.scaleX || c.scaleY !== canceller.scaleY) {
	        canceller.x = c.x;
	        canceller.y = c.y;
	...
	CameraCancellingE$1.CameraCancellingE = CameraCancellingE;
	...
	var Timer = /** @class */function () {
	  function Timer(interval, fps) {
	    ...
	    this.onElapse = new trigger_1$6.Trigger();
	...
	Object.defineProperty(TimerManager$1, "__esModule", {
	  value: true
	});
	TimerManager$1.TimerManager = TimerManager$1.TimerIdentifier = void 0;
	var ExceptionFactory_1$8 = ExceptionFactory$2;
	var Timer_1 = Timer$1;
	/**
	...
	TimerManager$1.TimerIdentifier = TimerIdentifier;
	/**
	 * Timerを管理する機構を提供する。
	...
	var TimerManager = /** @class */function () {
	  ...
	  TimerManager.prototype.createTimer = function (interval) {
	    ...
	    if (interval < 0) throw ExceptionFactory_1$8.ExceptionFactory.createAssertionError("TimerManager#createTimer: invalid interval");
	  ...
	  TimerManager.prototype.deleteTimer = function (timer) {
	    ...
	    if (index < 0) throw ExceptionFactory_1$8.ExceptionFactory.createAssertionError("TimerManager#deleteTimer: can not find timer");
	    this._timers.splice(index, 1);
	    timer.destroy();
	    if (!this._timers.length) {
	      if (!this._registered) throw ExceptionFactory_1$8.ExceptionFactory.createAssertionError("TimerManager#deleteTimer: handler is not handled");
	  ...
	  TimerManager.prototype._onTimeoutFired = function (identifier) {
	    ...
	    if (index < 0) throw ExceptionFactory_1$8.ExceptionFactory.createAssertionError("TimerManager#_onTimeoutFired: can not find identifier");
	  ...
	  TimerManager.prototype._clear = function (identifier) {
	    var index = this._identifiers.indexOf(identifier);
	    if (index < 0) throw ExceptionFactory_1$8.ExceptionFactory.createAssertionError("TimerManager#_clear: can not find identifier");
	    if (identifier.destroyed()) throw ExceptionFactory_1$8.ExceptionFactory.createAssertionError("TimerManager#_clear: invalid identifier");
	    this._identifiers.splice(index, 1);
	    var timer = identifier._timer;
	    identifier.destroy();
	    this.deleteTimer(timer);
	...
	TimerManager$1.TimerManager = TimerManager;

	Object.defineProperty(Scene$1, "__esModule", {
	  value: true
	});
	Scene$1.Scene = void 0;
	var trigger_1$5 = cjs;
	var AssetHolder_1 = AssetHolder$1;
	var Camera2D_1 = Camera2D$1;
	var ExceptionFactory_1$7 = ExceptionFactory$2;
	var TimerManager_1 = TimerManager$1;
	/**
	 * シーンを表すクラス。
	 */
	var Scene = /** @class */function () {
	  ...
	  function Scene(param) {
	    var game = param.game;
	    var local = param.local === undefined ? "non-local" : param.local === false ? "non-local" : param.local === true ? "full-local" : param.local;
	    ...
	    this.onLoad = new trigger_1$5.Trigger();
	    this.loaded = this.onLoad;
	    this._onReady = new trigger_1$5.Trigger();
	    ...
	    this.onUpdate = new trigger_1$5.Trigger();
	    this.update = this.onUpdate;
	    this._timer = new TimerManager_1.TimerManager(this.onUpdate, this.game.fps);
	    this.onAssetLoad = new trigger_1$5.Trigger();
	    this.onAssetLoadFailure = new trigger_1$5.Trigger();
	    this.onAssetLoadComplete = new trigger_1$5.Trigger();
	    ...
	    this.onMessage = new trigger_1$5.Trigger();
	    this.onPointDownCapture = new trigger_1$5.Trigger();
	    this.onPointMoveCapture = new trigger_1$5.Trigger();
	    this.onPointUpCapture = new trigger_1$5.Trigger();
	    this.onOperation = new trigger_1$5.Trigger();
	    ...
	    this.children = [];
	    this.state = "standby";
	    this.onStateChange = new trigger_1$5.Trigger();
	    this._assetHolders = [];
	  ...
	  Scene.prototype.gotoScene = function (next, toPush) {
	    if (!this.isCurrentScene()) throw ExceptionFactory_1$7.ExceptionFactory.createAssertionError("Scene#gotoScene: this scene is not the current scene");
	    if (toPush) {
	  ...
	  Scene.prototype.end = function () {
	    if (!this.isCurrentScene()) throw ExceptionFactory_1$7.ExceptionFactory.createAssertionError("Scene#end: this scene is not the current scene");
	    this.game.popScene();
	  };
	  /**
	   * このSceneにエンティティを登録する。
	  ...
	  Scene.prototype.requestAssets = function (assetIdsOrConf, handler) {
	    ...
	    if (this._loadingState !== "ready-fired" && this._loadingState !== "loaded-fired") {
	      ...
	      throw ExceptionFactory_1$7.ExceptionFactory.createAssertionError("Scene#requestAssets(): can be called after loaded.");
	    }
	    var assetIds;
	      ...
	      userData: function userData() {
	        ...
	        if (!_this.destroyed()) {
	          ...
	          if (failureAssetIds.length) {
	            ...
	            var error = ExceptionFactory_1$7.ExceptionFactory.createRequestAssetLoadError("Scene#requestAssets(): failed to load the asset. refer to the 'detail' property for more information.", {
	              failureAssetIds: failureAssetIds
	            });
	            handler(error);
	          } else {
	            handler();
	...
	Object.defineProperty(LoadingScene$1, "__esModule", {
	  value: true
	});
	LoadingScene$1.LoadingScene = void 0;
	var trigger_1$4 = cjs;
	var ExceptionFactory_1$6 = ExceptionFactory$2;
	var Scene_1$2 = Scene$1;
	/**
	...
	var LoadingScene = /** @class */function (_super) {
	  __extends$9(LoadingScene, _super);
	  /**
	  ...
	  function LoadingScene(param) {
	    ...
	    _this.onTargetReset = new trigger_1$4.Trigger();
	    _this.onTargetReady = new trigger_1$4.Trigger();
	    _this.onTargetAssetLoad = new trigger_1$4.Trigger();
	  ...
	  LoadingScene.prototype.end = function () {
	    if (!this._targetScene || this._targetScene._loadingState === "initial") {
	      ...
	      throw ExceptionFactory_1$6.ExceptionFactory.createAssertionError(msg);
	    }
	  ...
	  LoadingScene.prototype._clearTargetScene = function () {
	    ...
	    this.onLoad.removeAll({
	      owner: this,
	      func: this._doReset
	    });
	    this._targetScene._onReady.removeAll({
	      owner: this
	    });
	    this._targetScene.onAssetLoad.removeAll({
	      owner: this
	    });
	...
	LoadingScene$1.LoadingScene = LoadingScene;
	...
	Object.defineProperty(DefaultLoadingScene$1, "__esModule", {
	  value: true
	});
	DefaultLoadingScene$1.DefaultLoadingScene = void 0;
	var CameraCancellingE_1$1 = CameraCancellingE$1;
	var FilledRect_1$1 = FilledRect$1;
	var LoadingScene_1$1 = LoadingScene$1;
	/**
	 * デフォルトローディングシーン。
	 *
	...
	var DefaultLoadingScene = /** @class */function (_super) {
	  __extends$8(DefaultLoadingScene, _super);
	  /**
	  ...
	  DefaultLoadingScene.prototype._handleLoad = function () {
	    ...
	    var gauge;
	    this.append(new CameraCancellingE_1$1.CameraCancellingE({
	      scene: this,
	      children: [new FilledRect_1$1.FilledRect({
	        scene: this,
	        ...
	        children: [new FilledRect_1$1.FilledRect({
	          ...
	          cssColor: "gray",
	          children: [gauge = new FilledRect_1$1.FilledRect({
	            scene: this,
	            width: 0,
	            height: this._barHeight,
	            cssColor: "white"
	...
	}(LoadingScene_1$1.LoadingScene);
	DefaultLoadingScene$1.DefaultLoadingScene = DefaultLoadingScene;

	var DefaultSkippingScene$1 = {};
	...
	Object.defineProperty(DefaultSkippingScene$1, "__esModule", {
	  value: true
	});
	DefaultSkippingScene$1.DefaultSkippingScene = void 0;
	var CameraCancellingE_1 = CameraCancellingE$1;
	var FilledRect_1 = FilledRect$1;
	var Scene_1$1 = Scene$1;
	/**
	 * @ignore
	 */
	...
	var EasingFilledRect = /** @class */function (_super) {
	  __extends$7(EasingFilledRect, _super);
	  ...
	  EasingFilledRect.prototype._calculateCSSColor = function () {
	    var _a = this,
	      age = _a.age,
	      offsetDurationFrame = _a.offsetDurationFrame,
	      easingDurationFrame = _a.easingDurationFrame,
	      valueFrom = _a.valueFrom,
	      valueTo = _a.valueTo,
	      easing = _a.easing;
	...
	var DefaultSkippingScene = /** @class */function (_super) {
	  __extends$7(DefaultSkippingScene, _super);
	  /**
	  ...
	  function DefaultSkippingScene(param) {
	    var _this = _super.call(this, {
	      game: param.game,
	      local: "full-local",
	      name: "akashic:default-skipping-scene"
	    }) || this;
	...
	DefaultSkippingScene$1.DefaultSkippingScene = DefaultSkippingScene;

	var DynamicAssetConfiguration = {};

	Object.defineProperty(DynamicAssetConfiguration, "__esModule", {
	  value: true
	});

	var DynamicFont$1 = {};

	var SurfaceAtlasSet$1 = {};

	var SurfaceAtlas$1 = {};

	var SurfaceAtlasSlot$1 = {};

	Object.defineProperty(SurfaceAtlasSlot$1, "__esModule", {
	  value: true
	});
	SurfaceAtlasSlot$1.SurfaceAtlasSlot = void 0;
	/**
	...
	SurfaceAtlasSlot$1.SurfaceAtlasSlot = SurfaceAtlasSlot;

	Object.defineProperty(SurfaceAtlas$1, "__esModule", {
	  value: true
	});
	SurfaceAtlas$1.SurfaceAtlas = void 0;
	var SurfaceAtlasSlot_1 = SurfaceAtlasSlot$1;
	...
	SurfaceAtlas$1.SurfaceAtlas = SurfaceAtlas;

	Object.defineProperty(SurfaceAtlasSet$1, "__esModule", {
	  value: true
	});
	SurfaceAtlasSet$1.SurfaceAtlasSet = void 0;
	var SurfaceAtlas_1 = SurfaceAtlas$1;
	...
	SurfaceAtlasSet$1.SurfaceAtlasSet = SurfaceAtlasSet;
	...
	Object.defineProperty(DynamicFont$1, "__esModule", {
	  value: true
	});
	DynamicFont$1.DynamicFont = void 0;
	var pdi_types_1 = lib;
	var BitmapFont_1 = BitmapFont$1;
	var Font_1 = Font$1;
	var SurfaceAtlasSet_1$1 = SurfaceAtlasSet$1;
	var Util_1 = Util$1;
	/**
	 * ビットマップフォントを逐次生成するフォント。
	 */
	var DynamicFont = /** @class */function (_super) {
	  __extends$6(DynamicFont, _super);
	  /**
	  ...
	  function DynamicFont(param) {
	    ...
	    if (param.surfaceAtlasSet) {
	      ...
	      _this._atlasSet = new SurfaceAtlasSet_1$1.SurfaceAtlasSet({
	...
	DynamicFont$1.DynamicFont = DynamicFont;

	var EntityStateFlags = {};

	Object.defineProperty(EntityStateFlags, "__esModule", {
	  value: true
	});

	var EventConverter$1 = {};

	Object.defineProperty(EventConverter$1, "__esModule", {
	  value: true
	});
	EventConverter$1.EventConverter = void 0;
	var E_1 = E$1;
	var Event_1 = Event;
	var ExceptionFactory_1$5 = ExceptionFactory$2;
	/**
	 * 本クラスのインスタンスをゲーム開発者が直接生成することはなく、ゲーム開発者が利用する必要もない。
	 * @ignore
	 */
	var EventConverter = /** @class */function () {
	  ...
	  EventConverter.prototype.toGameEvent = function (pev) {
	    ...
	    // @ts-ignore
	    var player = this._playerTable[playerId] || {
	      id: playerId
	    };
	    switch (eventCode) {
	      ...
	      default:
	        // TODO handle error
	        throw ExceptionFactory_1$5.ExceptionFactory.createAssertionError("EventConverter#toGameEvent");
	    }
	  };
	  /**
	  ...
	  EventConverter.prototype.toPlaylogEvent = function (e, preservePlayer) {
	    ...
	    switch (e.type) {
	      ...
	      case "leave":
	        ...
	        throw ExceptionFactory_1$5.ExceptionFactory.createAssertionError("EventConverter#toPlaylogEvent: Invalid type: " + e.type);
	      case "timestamp":
	        var ts = e;
	        playerId = preservePlayer ? (_a = ts.player.id) !== null && _a !== void 0 ? _a : null : this._playerId;
	        return [2 /* pl.EventCode.Timestamp */, ts.eventFlags, playerId, ts.timestamp //            3: タイムスタンプ
	        ];

	      case "player-info":
	        var playerInfo = e;
	        playerId = preservePlayer ? (_b = playerInfo.player.id) !== null && _b !== void 0 ? _b : null : this._playerId;
	        return [3 /* pl.EventCode.PlayerInfo */, playerInfo.eventFlags, playerId, playerInfo.player.name, playerInfo.player.userData // 4: ユーザデータ
	        ];
	      ...
	      case "point-down":
	        ...
	        return [33 /* pl.EventCode.PointDown */, pointDown.eventFlags, playerId, pointDown.pointerId, pointDown.point.x, pointDown.point.y, targetId, pointDown.button, !!pointDown.local //       8?: ローカルイベントかどうか
	        ];
	      ...
	      case "point-move":
	        ...
	        return [34 /* pl.EventCode.PointMove */, pointMove.eventFlags, playerId, pointMove.pointerId, pointMove.point.x, pointMove.point.y, pointMove.startDelta.x, pointMove.startDelta.y, pointMove.prevDelta.x, pointMove.prevDelta.y, targetId, pointMove.button, !!pointMove.local //       12?: ローカルイベントかどうか
	        ];
	      ...
	      case "point-up":
	        ...
	        return [35 /* pl.EventCode.PointUp */, pointUp.eventFlags, playerId, pointUp.pointerId, pointUp.point.x, pointUp.point.y, pointUp.startDelta.x, pointUp.startDelta.y, pointUp.prevDelta.x, pointUp.prevDelta.y, targetId, pointUp.button, !!pointUp.local //       12?: ローカルイベントかどうか
	        ];

	      case "message":
	        var message = e;
	        playerId = preservePlayer && message.player ? (_f = message.player.id) !== null && _f !== void 0 ? _f : null : this._playerId;
	        return [32 /* pl.EventCode.Message */, message.eventFlags, playerId, message.data, !!message.local //       4?: ローカル
	        ];

	      case "operation":
	        var op = e;
	        playerId = preservePlayer && op.player ? (_g = op.player.id) !== null && _g !== void 0 ? _g : null : this._playerId;
	        return [64 /* pl.EventCode.Operation */, op.eventFlags, playerId, op.code, op.data, !!op.local //              5?: ローカル
	        ];

	      default:
	        throw ExceptionFactory_1$5.ExceptionFactory.createAssertionError("Unknown type: " + e.type);
	    }
	  };
	  EventConverter.prototype.makePlaylogOperationEvent = function (op) {
	    ...
	    return [64 /* pl.EventCode.Operation */, eventFlags, playerId, op._code, op.data, !!op.local //              5: ローカル
	    ];
	  };

	  return EventConverter;
	}();
	EventConverter$1.EventConverter = EventConverter;

	var EventFilter = {};

	Object.defineProperty(EventFilter, "__esModule", {
	  value: true
	});

	var EventFilterController = {};

	Object.defineProperty(EventFilterController, "__esModule", {
	  value: true
	});

	var EventIndex = {};

	Object.defineProperty(EventIndex, "__esModule", {
	  value: true
	});

	var EventPriority = {};

	Object.defineProperty(EventPriority, "__esModule", {
	  value: true
	});

	var GameMainParameterObject = {};

	Object.defineProperty(GameMainParameterObject, "__esModule", {
	  value: true
	});

	var InternalOperationPluginInfo = {};

	Object.defineProperty(InternalOperationPluginInfo, "__esModule", {
	  value: true
	});

	var LocalTickModeString = {};

	Object.defineProperty(LocalTickModeString, "__esModule", {
	  value: true
	});

	var ModuleManager$1 = {};

	var RequireCachedValue$1 = {};

	Object.defineProperty(RequireCachedValue$1, "__esModule", {
	  value: true
	});
	RequireCachedValue$1.RequireCachedValue = void 0;
	...
	RequireCachedValue$1.RequireCachedValue = RequireCachedValue;

	var ScriptAssetContext$1 = {};

	Object.defineProperty(ScriptAssetContext$1, "__esModule", {
	  value: true
	});
	ScriptAssetContext$1.ScriptAssetContext = void 0;
	var ExceptionFactory_1$4 = ExceptionFactory$2;
	/**
	...
	var ScriptAssetContext = /** @class */function () {
	  ...
	  ScriptAssetContext.prototype._cachedValue = function () {
	    if (!this._started) throw ExceptionFactory_1$4.ExceptionFactory.createAssertionError("ScriptAssetContext#_cachedValue: not executed yet.");
	...
	ScriptAssetContext$1.ScriptAssetContext = ScriptAssetContext;

	Object.defineProperty(ModuleManager$1, "__esModule", {
	  value: true
	});
	ModuleManager$1.ModuleManager = void 0;
	var PathUtil_1$1 = PathUtil$3;
	var ExceptionFactory_1$3 = ExceptionFactory$2;
	var Module_1 = Module$1;
	var RequireCachedValue_1 = RequireCachedValue$1;
	var ScriptAssetContext_1 = ScriptAssetContext$1;
	/**
	 * `Module` を管理するクラス。
	...
	var ModuleManager = /** @class */function () {
	  ...
	  ModuleManager.prototype._require = function (path, currentModule) {
	    ...
	    if (targetScriptAsset) {
	      ...
	      if (targetScriptAsset.type === "script") {
	        var module = new Module_1.Module({
	          ...
	          requireFunc: function requireFunc(path, mod) {
	            return _this._require(path, mod);
	          },
	          resolveFunc: function resolveFunc(path, mod) {
	            return _this._resolvePath(path, mod);
	          }
	        });
	        ...
	        if (targetScriptAsset && PathUtil_1$1.PathUtil.resolveExtname(path) === ".json") {
	    ...
	    throw ExceptionFactory_1$3.ExceptionFactory.createAssertionError("g._require: can not find module: " + path);
	  };
	  /**
	  ...
	  ModuleManager.prototype._resolvePath = function (path, currentModule) {
	    ...
	    if (/^\.\/|^\.\.\/|^\//.test(path)) {
	      ...
	      if (currentModule) {
	        if (!currentModule._virtualDirname) {
	          throw ExceptionFactory_1$3.ExceptionFactory.createAssertionError("g._require.resolve: couldn't resolve the moudle path without virtualPath");
	        }
	        resolvedPath = PathUtil_1$1.PathUtil.resolvePath(currentModule._virtualDirname, path);
	      } else {
	        if (!/^\.\//.test(path)) {
	          throw ExceptionFactory_1$3.ExceptionFactory.createAssertionError("g._require.resolve: entry point path must start with './'");
	        }
	      ...
	      for (var i = 0; i < dirs.length; ++i) {
	        var dir = dirs[i];
	        var targetPath = PathUtil_1$1.PathUtil.resolvePath(dir, path);
	    ...
	    throw ExceptionFactory_1$3.ExceptionFactory.createAssertionError("g._require.resolve: couldn't resolve the path: " + path);
	  };
	  /**
	  ...
	  ModuleManager.prototype._resolveAbsolutePathAsDirectory = function (resolvedPath, liveAssetPathTable) {
	    ...
	    if (liveAssetPathTable.hasOwnProperty(path) && asset.type === "text") {
	      ...
	      if (pkg && typeof pkg.main === "string") {
	        var targetPath = this._resolveAbsolutePathAsFile(PathUtil_1$1.PathUtil.resolvePath(resolvedPath, pkg.main), liveAssetPathTable);
	        if (targetPath) {
	          return targetPath;
	        }
	      }
	...
	ModuleManager$1.ModuleManager = ModuleManager;

	var NinePatchSurfaceEffector$1 = {};

	Object.defineProperty(NinePatchSurfaceEffector$1, "__esModule", {
	  value: true
	});
	NinePatchSurfaceEffector$1.NinePatchSurfaceEffector = void 0;
	var SurfaceUtil_1 = SurfaceUtil$1;
	/**
	...
	NinePatchSurfaceEffector$1.NinePatchSurfaceEffector = NinePatchSurfaceEffector;

	var OperationPlugin = {};

	Object.defineProperty(OperationPlugin, "__esModule", {
	  value: true
	});

	var OperationPluginManager$1 = {};
	...
	Object.defineProperty(OperationPluginManager$1, "__esModule", {
	  value: true
	});
	OperationPluginManager$1.OperationPluginManager = void 0;
	var trigger_1$3 = cjs;
	/**
	...
	var OperationPluginManager = /** @class */function () {
	  function OperationPluginManager(game, viewInfo) {
	    this.onOperate = new trigger_1$3.Trigger();
	...
	OperationPluginManager$1.OperationPluginManager = OperationPluginManager;

	var OperationPluginOperation = {};

	Object.defineProperty(OperationPluginOperation, "__esModule", {
	  value: true
	});

	var OperationPluginStatic = {};

	Object.defineProperty(OperationPluginStatic, "__esModule", {
	  value: true
	});
	...
	var PointEventResolver$1 = {};

	Object.defineProperty(PointEventResolver$1, "__esModule", {
	  value: true
	});
	PointEventResolver$1.PointEventResolver = void 0;
	/**
	...
	var PointEventResolver = /** @class */function () {
	  ...
	  PointEventResolver.prototype.pointDown = function (e) {
	    ...
	    var ret = [33 /* pl.EventCode.PointDown */, 2 /* EventPriority.Joined */, this._playerId, e.identifier, point.x, point.y, targetId, e.button //                7?: ボタンの種類
	    ];
	  ...
	  PointEventResolver.prototype.pointMove = function (e) {
	    ...
	    var ret = [34 /* pl.EventCode.PointMove */, 2 /* EventPriority.Joined */, this._playerId, e.identifier, holder.point.x, holder.point.y, start.x, start.y, prev.x, prev.y, holder.targetId, e.button //                11?: ボタンの種類
	    ];
	  ...
	  PointEventResolver.prototype.pointUp = function (e) {
	    ...
	    this._currentPoints--;
	    var ret = [35 /* pl.EventCode.PointUp */, 2 /* EventPriority.Joined */, this._playerId, e.identifier, holder.point.x, holder.point.y, start.x, start.y, prev.x, prev.y, holder.targetId, e.button //              11?: ボタンの種類
	    ];
	...
	PointEventResolver$1.PointEventResolver = PointEventResolver;

	var RandomGenerator$1 = {};

	Object.defineProperty(RandomGenerator$1, "__esModule", {
	  value: true
	});
	...
	Object.defineProperty(Require, "__esModule", {
	  value: true
	});

	var RequireCacheable = {};

	Object.defineProperty(RequireCacheable, "__esModule", {
	  value: true
	});

	var SnapshotSaveRequest = {};

	Object.defineProperty(SnapshotSaveRequest, "__esModule", {
	  value: true
	});

	var SpriteFactory$1 = {};

	Object.defineProperty(SpriteFactory$1, "__esModule", {
	  value: true
	});
	SpriteFactory$1.SpriteFactory = void 0;
	var Sprite_1 = Sprite$1;
	var ExceptionFactory_1$2 = ExceptionFactory$2;
	var SpriteFactory = /** @class */function () {
	  ...
	  SpriteFactory.createSpriteFromE = function (scene, e, camera) {
	    ...
	    if (!boundingRect) {
	      throw ExceptionFactory_1$2.ExceptionFactory.createAssertionError("SpriteFactory.createSpriteFromE: camera must look e");
	    }
	...
	SpriteFactory$1.SpriteFactory = SpriteFactory;

	var SurfaceEffector = {};

	Object.defineProperty(SurfaceEffector, "__esModule", {
	  value: true
	});

	var TextAlignString = {};

	Object.defineProperty(TextAlignString, "__esModule", {
	  value: true
	});

	var TextMetrics = {};

	Object.defineProperty(TextMetrics, "__esModule", {
	  value: true
	});

	var TickGenerationModeString = {};

	Object.defineProperty(TickGenerationModeString, "__esModule", {
	  value: true
	});
	...
	Object.defineProperty(Xorshift$1, "__esModule", {
	  value: true
	});
	Xorshift$1.Xorshift = void 0;
	...
	Xorshift$1.Xorshift = Xorshift;

	var XorshiftRandomGenerator$1 = {};
	...
	Object.defineProperty(XorshiftRandomGenerator$1, "__esModule", {
	  value: true
	});
	XorshiftRandomGenerator$1.XorshiftRandomGenerator = void 0;
	var ExceptionFactory_1$1 = ExceptionFactory$2;
	var RandomGenerator_1 = RandomGenerator$1;
	var Xorshift_1 = Xorshift$1;
	/**
	 * Xorshiftを用いた乱数生成期。
	 */
	var XorshiftRandomGenerator = /** @class */function (_super) {
	  __extends$5(XorshiftRandomGenerator, _super);
	  function XorshiftRandomGenerator(seed, xorshift) {
	    var _this = this;
	    if (seed === undefined) {
	      throw ExceptionFactory_1$1.ExceptionFactory.createAssertionError("XorshiftRandomGenerator#constructor: seed is undefined");
	    } else {
	...
	XorshiftRandomGenerator$1.XorshiftRandomGenerator = XorshiftRandomGenerator;

	var Game$3 = {};

	Object.defineProperty(Game$3, "__esModule", {
	  value: true
	});
	Game$3.Game = void 0;
	var trigger_1$2 = cjs;
	var AssetAccessor_1 = AssetAccessor$1;
	var AssetManager_1 = AssetManager$1;
	var AudioSystemManager_1 = AudioSystemManager$1;
	var DefaultLoadingScene_1 = DefaultLoadingScene$1;
	var DefaultSkippingScene_1 = DefaultSkippingScene$1;
	var EventConverter_1 = EventConverter$1;
	var ExceptionFactory_1 = ExceptionFactory$2;
	var LoadingScene_1 = LoadingScene$1;
	var ModuleManager_1 = ModuleManager$1;
	var OperationPluginManager_1 = OperationPluginManager$1;
	var PointEventResolver_1 = PointEventResolver$1;
	var Scene_1 = Scene$1;
	var SurfaceAtlasSet_1 = SurfaceAtlasSet$1;
	var WeakRefKVS_1 = WeakRefKVS$1;
	var XorshiftRandomGenerator_1 = XorshiftRandomGenerator$1;
	/**
	 * コンテンツそのものを表すクラス。
	 *
	...
	var Game$2 = /** @class */function () {
	  ...
	  function Game(param) {
	    ...
	    this.assets = {};
	    this.surfaceAtlasSet = new SurfaceAtlasSet_1.SurfaceAtlasSet({
	      resourceFactory: this.resourceFactory
	    });
	    this.onJoin = new trigger_1$2.Trigger();
	    this.onLeave = new trigger_1$2.Trigger();
	    this.onPlayerInfo = new trigger_1$2.Trigger();
	    this.onSeed = new trigger_1$2.Trigger();
	    ...
	    this.onResized = new trigger_1$2.Trigger();
	    this.onSkipChange = new trigger_1$2.Trigger();
	    ...
	    this._onLoad = new trigger_1$2.Trigger();
	    this._onStart = new trigger_1$2.Trigger();
	    this._loaded = this._onLoad;
	    this._started = this._onStart;
	    this.isLoaded = false;
	    this.onSnapshotRequest = new trigger_1$2.Trigger();
	    ...
	    this._onOperationPluginOperated = new trigger_1$2.Trigger();
	    ...
	    this.onSceneChange = new trigger_1$2.Trigger();
	    this._onSceneChange = new trigger_1$2.Trigger();
	    this._onSceneChange.add(this._handleSceneChanged, this);
	    this._sceneChanged = this._onSceneChange;
	    this.onUpdate = new trigger_1$2.Trigger();
	  ...
	  Game.prototype.replaceScene = function (scene, preserveCurrentOrOption) {
	    var preserveCurrent;
	    var prepare;
	    if (_typeof(preserveCurrentOrOption) === "object") {
	  ...
	  Game.prototype.popScene = function (preserve, step) {
	    ...
	    for (var i = 0; i < step; i++) {
	      this._postTickTasks.push({
	        type: 2 /* PostTickTaskType.PopScene */,
	        preserveCurrent: !!preserve
	      });
	    }
	  };
	  /**
	  ...
	  Game.prototype.render = function () {
	    ...
	    for (var i = 0; i < renderers.length; ++i) {
	      ...
	      if (scene === skippingScene) {
	        for (var k = 0; k < scene.children.length; ++k) scene.children[k].render(renderer, camera);
	      } else {
	        for (var j = renderBottomIndex; j < this.scenes.length; ++j) {
	          var children = this.scenes[j].children;
	          for (var k = 0; k < children.length; ++k) children[k].render(renderer, camera);
	        }
	      }
	      if (camera) {
	        renderer.restore();
	  ...
	  Game.prototype.register = function (e) {
	    if (e.local) {
	      if (e.id === undefined) {
	        ...
	        if (e.id > 0) throw ExceptionFactory_1.ExceptionFactory.createAssertionError("Game#register: invalid local id: " + e.id);
	        if (this._localDb.has(e.id)) throw ExceptionFactory_1.ExceptionFactory.createAssertionError("Game#register: conflicted id: " + e.id);
	      ...
	      if (e.id === undefined) {
	        ...
	        // register前にidがある: スナップショットからの復元用パス
	        if (e.id < 0) throw ExceptionFactory_1.ExceptionFactory.createAssertionError("Game#register: invalid non-local id: " + e.id);
	        if (this.db.has(e.id)) throw ExceptionFactory_1.ExceptionFactory.createAssertionError("Game#register: conflicted id: " + e.id);
	  ...
	  Game.prototype.requestSaveSnapshot = function (func, owner) {
	    ...
	    this._postTickTasks.unshift({
	      ...
	      fun: function fun() {
	        if (!_this.handlerSet.shouldSaveSnapshot()) return;
	  ...
	  Game.prototype._popSceneRaw = function (preserveCurrent) {
	    this._postTickTasks.push({
	      type: 4 /* PostTickTaskType.PopSceneRaw */,
	      preserveCurrent: preserveCurrent
	    });
	  };
	  /**
	  ...
	  Game.prototype._normalizeConfiguration = function (gameConfiguration) {
	    if (!gameConfiguration) throw ExceptionFactory_1.ExceptionFactory.createAssertionError("Game#_normalizeConfiguration: invalid arguments");
	    if (gameConfiguration.assets == null) gameConfiguration.assets = {};
	    if (gameConfiguration.fps == null) gameConfiguration.fps = 30;
	    if (typeof gameConfiguration.fps !== "number") throw ExceptionFactory_1.ExceptionFactory.createAssertionError("Game#_normalizeConfiguration: fps must be given as a number");
	    if (!(0 <= gameConfiguration.fps && gameConfiguration.fps <= 60)) throw ExceptionFactory_1.ExceptionFactory.createAssertionError("Game#_normalizeConfiguration: fps must be a number in (0, 60].");
	    if (typeof gameConfiguration.width !== "number") throw ExceptionFactory_1.ExceptionFactory.createAssertionError("Game#_normalizeConfiguration: width must be given as a number");
	    if (typeof gameConfiguration.height !== "number") throw ExceptionFactory_1.ExceptionFactory.createAssertionError("Game#_normalizeConfiguration: height must be given as a number");
	  ...
	  Game.prototype._reset = function (param) {
	    ...
	    this.audio._reset();
	    this._onLoad.removeAll({
	      func: this._handleLoad,
	      owner: this
	    });
	    ...
	    switch (this._configuration.defaultLoadingScene) {
	      case "none":
	        ...
	        this._defaultLoadingScene = new LoadingScene_1.LoadingScene({
	          game: this
	        });
	        break;
	      case "compact":
	        this._defaultLoadingScene = new DefaultLoadingScene_1.DefaultLoadingScene({
	          game: this,
	          style: "compact"
	        });
	        break;
	    ...
	    switch (this._configuration.defaultSkippingScene) {
	      case "none":
	        this._defaultSkippingScene = new DefaultSkippingScene_1.DefaultSkippingScene({
	          game: this,
	          style: "none"
	        });
	        break;
	  ...
	  Game.prototype._startLoadingGlobalAssets = function () {
	    if (this.isLoaded) throw ExceptionFactory_1.ExceptionFactory.createAssertionError("Game#_startLoadingGlobalAssets: already loaded.");
	    this.pushScene(this._initialScene);
	    this._flushPostTickTasks();
	  };
	  /**
	      ...
	      for (var i = 0; i < reqs.length; ++i) {
	        ...
	        switch (req.type) {
	          case 0 /* PostTickTaskType.PushScene */:
	            ...
	            this._doPushScene(req.scene, false, req.prepare ? this._createPreparingLoadingScene(req.scene, req.prepare, "akashic:preparing-".concat(req.scene.name)) : undefined);
	            break;
	  ...
	  Game.prototype._handleLeaveEvent = function (event) {
	    this.joinedPlayerIds = this.joinedPlayerIds.filter(function (id) {
	      return id !== event.player.id;
	    });
	  };
	  /**
	   * シーンスタックからシーンを取り除く。
	  ...
	  Game.prototype._doPopScene = function (preserveCurrent, raw, fireSceneChanged) {
	    ...
	    if (!scene) throw ExceptionFactory_1.ExceptionFactory.createAssertionError("Game#_doPopScene: invalid call; scene stack underflow");
	    if (scene === this._initialScene) throw ExceptionFactory_1.ExceptionFactory.createAssertionError("Game#_doPopScene: invalid call; attempting to pop the initial scene");
	    if (!preserveCurrent) {
	    ...
	    if (!raw) {
	      ...
	      if (nextScene && nextScene._needsLoading() && nextScene._loadingState !== "loaded-fired") {
	        var loadingScene = nextScene._waitingPrepare ? this._createPreparingLoadingScene(nextScene, nextScene._waitingPrepare, "akashic:preparing-".concat(nextScene.name)) : (_a = this.loadingScene) !== null && _a !== void 0 ? _a : this._defaultLoadingScene;
	  ...
	  Game.prototype._handleLoad = function () {
	    ...
	    for (var _a = 0, preloadAssetIds_1 = preloadAssetIds; _a < preloadAssetIds_1.length; _a++) {
	      ...
	      if (!fun || typeof fun !== "function") throw ExceptionFactory_1.ExceptionFactory.createAssertionError("Game#_handleLoad: ".concat(preloadAssetId, " has no-exported function."));
	      fun();
	    }
	    if (this._mainFunc) {
	      ...
	      if (!mainFun || typeof mainFun !== "function") throw ExceptionFactory_1.ExceptionFactory.createAssertionError("Game#_handleLoad: Entry point ".concat(this._main, " not found."));
	  ...
	  Game.prototype._doPushScene = function (scene, raw, loadingScene) {
	    ...
	    if (scene._needsLoading() && scene._loadingState !== "loaded-fired") {
	      if (this._defaultLoadingScene._needsLoading()) throw ExceptionFactory_1.ExceptionFactory.createAssertionError("Game#_doPushScene: _defaultLoadingScene must not depend on any assets.");
	      this._doPushScene(loadingScene, true, this._defaultLoadingScene);
	...
	(function (exports) {
	  ...
	  Object.defineProperty(exports, "AudioSystem", {
	    enumerable: true,
	    get: function get() {
	      return AudioSystem_1.AudioSystem;
	    }
	  });
	  var Module_1 = Module$1;
	  ...
	  var ShaderProgram_1 = ShaderProgram$1;
	  Object.defineProperty(exports, "ShaderProgram", {
	    enumerable: true,
	    get: function get() {
	      return ShaderProgram_1.ShaderProgram;
	    }
	  });
	  var VideoSystem_1 = VideoSystem$1;
	  Object.defineProperty(exports, "VideoSystem", {
	    enumerable: true,
	    get: function get() {
	      return VideoSystem_1.VideoSystem;
	    }
	  });
	  // 後方互換性のため PathUtil のみ reexport する。
	  var PathUtil_1 = PathUtil$3;
	  Object.defineProperty(exports, "PathUtil", {
	    enumerable: true,
	    get: function get() {
	      return PathUtil_1.PathUtil;
	    }
	  });
	  __exportStar(CacheableE$1, exports);
	  __exportStar(E$1, exports);
	  __exportStar(FilledRect$1, exports);
	  __exportStar(FrameSprite$1, exports);
	  __exportStar(Label$1, exports);
	  __exportStar(Pane$1, exports);
	  __exportStar(Sprite$1, exports);
	  __exportStar(AssetAccessor$1, exports);
	  __exportStar(AssetGenerationConfiguration, exports);
	  __exportStar(AssetHolder$1, exports);
	  __exportStar(AssetLoadFailureInfo, exports);
	  __exportStar(AssetManager$1, exports);
	  __exportStar(AssetManagerLoadHandler, exports);
	  __exportStar(AudioPlayContext$1, exports);
	  __exportStar(AudioSystem$1, exports);
	  __exportStar(AudioSystemManager$1, exports);
	  __exportStar(AudioUtil$1, exports);
	  __exportStar(BitmapFont$1, exports);
	  __exportStar(Camera, exports);
	  __exportStar(Camera2D$1, exports);
	  __exportStar(Collision$1, exports);
	  __exportStar(DefaultLoadingScene$1, exports);
	  __exportStar(DefaultSkippingScene$1, exports);
	  __exportStar(DynamicAssetConfiguration, exports);
	  __exportStar(DynamicFont$1, exports);
	  __exportStar(EntityStateFlags, exports);
	  __exportStar(Event, exports);
	  __exportStar(EventConverter$1, exports);
	  __exportStar(EventFilter, exports);
	  __exportStar(EventFilterController, exports);
	  __exportStar(EventIndex, exports);
	  __exportStar(EventPriority, exports);
	  __exportStar(ExceptionFactory$2, exports);
	  __exportStar(Font$1, exports);
	  __exportStar(GameMainParameterObject, exports);
	  __exportStar(InternalOperationPluginInfo, exports);
	  __exportStar(LoadingScene$1, exports);
	  __exportStar(LocalTickModeString, exports);
	  __exportStar(Matrix, exports);
	  __exportStar(Module$1, exports);
	  __exportStar(ModuleManager$1, exports);
	  __exportStar(NinePatchSurfaceEffector$1, exports);
	  __exportStar(Object2D$1, exports);
	  __exportStar(OperationPlugin, exports);
	  __exportStar(OperationPluginManager$1, exports);
	  __exportStar(OperationPluginOperation, exports);
	  __exportStar(OperationPluginStatic, exports);
	  __exportStar(Player, exports);
	  __exportStar(PointEventResolver$1, exports);
	  __exportStar(RandomGenerator$1, exports);
	  __exportStar(Require, exports);
	  __exportStar(RequireCacheable, exports);
	  __exportStar(RequireCachedValue$1, exports);
	  __exportStar(ScriptAssetContext$1, exports);
	  __exportStar(ShaderProgram$1, exports);
	  __exportStar(SnapshotSaveRequest, exports);
	  __exportStar(SpriteFactory$1, exports);
	  __exportStar(SurfaceAtlas$1, exports);
	  __exportStar(SurfaceAtlasSet$1, exports);
	  __exportStar(SurfaceAtlasSlot$1, exports);
	  __exportStar(SurfaceEffector, exports);
	  __exportStar(SurfaceUtil$1, exports);
	  __exportStar(TextAlign$1, exports);
	  __exportStar(TextAlignString, exports);
	  __exportStar(TextMetrics, exports);
	  __exportStar(TickGenerationModeString, exports);
	  __exportStar(Timer$1, exports);
	  __exportStar(TimerManager$1, exports);
	  __exportStar(Util$1, exports);
	  __exportStar(VideoSystem$1, exports);
	  __exportStar(WeakRefKVS$1, exports);
	  __exportStar(Xorshift$1, exports);
	  __exportStar(XorshiftRandomGenerator$1, exports);
	  __exportStar(Scene$1, exports);
	  __exportStar(Game$3, exports);
	})(index_common);

	var GameHandlerSet$2 = {};

	Object.defineProperty(GameHandlerSet$2, "__esModule", {
	  value: true
	});
	...
	(function (exports) {
	  ...
	  __exportStar(index_common, exports);
	  __exportStar(GameHandlerSet$2, exports); // NOTE: コンテンツから参照する必要はない
	})(lib$2);

	var akashicEngine$1 = lib$2;

	Object.defineProperty(SimpleProfiler$1, "__esModule", {
	  value: true
	});
	SimpleProfiler$1.SimpleProfiler = void 0;
	var g$8 = akashicEngine$1;
	var SimpleProfiler = /** @class */function () {
	  function SimpleProfiler(param) {
	    ...
	    this._beforeTimes = [];
	    this._values = [];
	    this._calculateProfilerValueTrigger = new g$8.Trigger();
	  ...
	  SimpleProfiler.prototype.flush = function () {
	    ...
	    if (this._values[1 /* ProfilerValueType.RawFrameInterval */].length > this._limit) {
	      for (var i in this._values) {
	        if (this._values.hasOwnProperty(i)) this._values[i] = this._values[i].slice(-SimpleProfiler.BACKUP_MARGIN);
	      }
	    }
	  };
	...
	SimpleProfiler$1.SimpleProfiler = SimpleProfiler;

	var ExecutionMode$1 = {};

	Object.defineProperty(ExecutionMode$1, "__esModule", {
	  value: true
	});
	/**
	 * `GameLoop` の実行モード。
	...
	ExecutionMode$1["default"] = ExecutionMode;
	...
	Object.defineProperty(Game$1, "__esModule", {
	  value: true
	});
	Game$1.Game = void 0;
	var g$7 = akashicEngine$1;
	/**
	 * Gameクラス。
	 *
	...
	var Game = /** @class */function (_super) {
	  ...
	  function Game(param) {
	     ...
	     * fire時には到達したageが渡される。
	     */
	    _this.agePassedTrigger = new g$7.Trigger();
	    /**
	     * 要求を受けた後の目標時刻到達を通知するTrigger。
	     * 目標時刻関数を用いたリプレイ中でなければfireされない。
	     * fire時には到達した目標時刻が渡される。
	     */
	    _this.targetTimeReachedTrigger = new g$7.Trigger();
	    /**
	     ...
	     */
	    _this.skippingChangedTrigger = new g$7.Trigger();
	    /**
	     ...
	     */
	    _this.abortTrigger = new g$7.Trigger();
	    ...
	    _this.rawHandlerSet = param.handlerSet;
	...
	Object.defineProperty(PathUtil$1, "__esModule", {
	  value: true
	});
	PathUtil$1.PathUtil = void 0;
	/**
	 * パスユーティリティ。
	 */
	var PathUtil;
	(function (PathUtil) {
	  ...
	  PathUtil.splitPath = splitPath;
	})(PathUtil || (PathUtil$1.PathUtil = PathUtil = {}));
	...
	utils._mergeObject = utils.toAssetArray = utils.makeLoadConfigurationFunc = void 0;
	var PathUtil_1 = PathUtil$1;
	...
	function makeLoadConfigurationFunc(loadConfiguration) {
	  function loadResolvedConfiguration(url, assetBase, cascadeBase, callback) {
	    loadConfiguration(url, function (err, conf) {
	      ...
	      Promise.all(defs).then(function (confs) {
	        return callback(null, confs.reduce(_mergeObject));
	      })["catch"](function (e) {
	        return callback(e);
	      });
	    });
	  ...
	  function promisifiedLoad(url, assetBase, cascadeBase) {
	    return new Promise(function (resolve, reject) {
	      loadResolvedConfiguration(url, assetBase, cascadeBase, function (err, conf) {
	        return err ? reject(err) : resolve(conf);
	      });
	    });
	  }
	  return loadResolvedConfiguration;
	}
	utils.makeLoadConfigurationFunc = makeLoadConfigurationFunc;
	/**
	 * アセット定義のマップを配列に変換する。
	...
	utils.toAssetArray = toAssetArray;
	/**
	...
	function _mergeObject(target, source) {
	  ...
	  for (var i = 0, len = ks.length; i < len; ++i) {
	    ...
	    var sourceValType = _typeof(sourceVal);
	    var targetValType = _typeof(target[k]);
	...
	utils._mergeObject = _mergeObject;
	/**
	 * @private
	 */
	function _normalizeAssets(configuration, assetBase) {
	  ...
	  } else if (_typeof(configuration.assets) === "object") {
	...
	(function (exports) {
	  ...
	  __exportStar(PathUtil$1, exports);
	...
	Object.defineProperty(EventBuffer$1, "__esModule", {
	  value: true
	});
	EventBuffer$1.EventBuffer = void 0;
	/**
	...
	var EventBuffer = /** @class */function () {
	  function EventBuffer(param) {
	    ...
	    this._filterController = {
	      ...
	      processNext: function processNext(pev) {
	  ...
	  EventBuffer.isEventLocal = function (pev) {
	    switch (pev[0 /* g.EventIndex.General.Code */]) {
	      ...
	      case 34 /* pl.EventCode.PointMove */:
	        return pev[12 /* g.EventIndex.PointMove.Local */];
	      case 35 /* pl.EventCode.PointUp */:
	        return pev[12 /* g.EventIndex.PointUp.Local */];
	      case 64 /* pl.EventCode.Operation */:
	  ...
	  EventBuffer.prototype.onEvent = function (pev) {
	    if (EventBuffer.isEventLocal(pev)) {
	      if (this._isLocalReceiver && !this._isDiscarder && !(this._skipping && this._discardsLocalEventsDuringSkip)) {
	  ...
	  EventBuffer.prototype.removeFilter = function (filter) {
	    ...
	    for (var i = this._filters.length - 1; i >= 0; --i) {
	      if (this._filters[i].func === filter) this._filters.splice(i, 1);
	    }
	  };
	  EventBuffer.prototype.processEvents = function (isLocal) {
	    ...
	    if (this._filters) {
	      for (var i = 0; i < this._filters.length; ++i) {
	        ...
	        if (pevs.length > 0 || filter.handleEmpty) pevs = this._filters[i].func(pevs, this._filterController) || [];
	      }
	    }
	...
	EventBuffer$1.EventBuffer = EventBuffer;

	var GameHandlerSet$1 = {};

	Object.defineProperty(GameHandlerSet$1, "__esModule", {
	  value: true
	});
	GameHandlerSet$1.GameHandlerSet = void 0;
	var g$6 = akashicEngine$1;
	var GameHandlerSet = /** @class */function () {
	  function GameHandlerSet(param) {
	    this.raiseEventTrigger = new g$6.Trigger();
	    this.raiseTickTrigger = new g$6.Trigger();
	    this.snapshotTrigger = new g$6.Trigger();
	    this.changeSceneModeTrigger = new g$6.Trigger();
	  ...
	  GameHandlerSet.prototype.removeEventFilter = function (filter) {
	    if (this._eventFilterFuncs && filter)
	      // `filter` のチェックは念のため。(undefined で呼ぶと removeFilter() が全削除になってしまう)
	  ...
	  GameHandlerSet.prototype.saveSnapshot = function (frame, gameSnapshot, randGenSer, nextEntityId, timestamp) {
	    if (timestamp === void 0) {
	      timestamp = this._getCurrentTimeFunc();
	    }
	...
	GameHandlerSet$1.GameHandlerSet = GameHandlerSet;
	...
	var Clock = /** @class */function () {
	  function Clock(param) {
	    ...
	    this.frameTrigger = new g$5.Trigger();
	    this.rawFrameTrigger = new g$5.Trigger();
	  ...
	  Clock.prototype._onLooperCall = function (deltaTime) {
	    ...
	    var frameCount = totalDeltaTime < this._waitTimeDoubled ? 1 : totalDeltaTime > this._waitTimeMax ? this._realMaxFramePerOnce : totalDeltaTime / this._waitTime | 0;
	    var fc = frameCount;
	    var arg = {
	...
	Object.defineProperty(LoopMode$1, "__esModule", {
	  value: true
	});
	/**
	 * `GameLoop` のループ制御のモード。
	...
	var LoopRenderMode$1 = {};

	Object.defineProperty(LoopRenderMode$1, "__esModule", {
	  value: true
	});
	/**
	 * `GameLoop` が描画を行う基準。
	...
	LoopRenderMode$1["default"] = LoopRenderMode;
	...
	Object.defineProperty(ProfilerClock$1, "__esModule", {
	  value: true
	});
	ProfilerClock$1.ProfilerClock = void 0;
	var Clock_1$1 = Clock$1;
	/**
	 * プロファイラーを有するクロック。
	 *
	...
	var ProfilerClock = /** @class */function (_super) {
	  __extends$3(ProfilerClock, _super);
	...
	ProfilerClock$1.ProfilerClock = ProfilerClock;

	var TickController$1 = {};

	var TickBuffer$1 = {};

	Object.defineProperty(TickBuffer$1, "__esModule", {
	  value: true
	});
	TickBuffer$1.TickBuffer = void 0;
	var g$4 = akashicEngine$1;
	var ExecutionMode_1$3 = ExecutionMode$1;
	g$4.EventIndex; // eslint-disable-line @typescript-eslint/naming-convention
	/**
	...
	var TickBuffer = /** @class */function () {
	  ...
	  TickBuffer.prototype.setExecutionMode = function (execMode) {
	    ...
	    if (this._executionMode === execMode) return;
	  ...
	  TickBuffer.prototype.setCurrentAge = function (age) {
	    ...
	    this._nextTickTimeCache = null;
	    this.currentAge = age;
	    if (this.knownLatestAge < age - 1) this.knownLatestAge = age - 1;
	  ...
	  TickBuffer.prototype.consume = function () {
	    ...
	    if (age === range.start) {
	      ...
	      if (range.start === range.end) this._tickRanges.shift();
	  ...
	  TickBuffer.prototype.readNextTickTime = function () {
	    if (this._nextTickTimeCache != null) return this._nextTickTimeCache;
	    if (this.currentAge === this._nearestAbsentAge) return null;
	    var age = this.currentAge;
	    var range = this._tickRanges[0];
	    if (age === range.start) {
	      if (range.ticks.length === 0) return null;
	      var tick = range.ticks[0];
	      if (tick[0 /* EventIndex.Tick.Age */] !== age) return null;
	      ...
	      for (var i = 0; i < pevs.length; ++i) {
	        if (pevs[i][0 /* EventIndex.General.Code */] === 2 /* pl.EventCode.Timestamp */) {
	          ...
	          if (nextTickTime < this._oldTimestampThreshold) nextTickTime += this._startedAt;
	  ...
	  TickBuffer.prototype.requestTicks = function (from, len) {
	    if (from === void 0) {
	      from = this.currentAge;
	    }
	    if (len === void 0) {
	      len = this._sizeRequestOnce;
	    }
	    if (this._skipping) {
	  ...
	  TickBuffer.prototype.requestAllTicks = function (from, len) {
	    ...
	    if (this._executionMode !== ExecutionMode_1$3["default"].Passive) return;
	  ...
	  TickBuffer.prototype.addTick = function (tick) {
	    ...
	    if (gotNext) this.gotNextTickTrigger.fire();
	  };
	  TickBuffer.prototype.addTickList = function (tickList) {
	    ...
	    if (i > 0) {
	      ...
	      if (start < leftEndAge) start = leftEndAge;
	    }
	    ...
	    if (i < len) {
	      ...
	      if (end > rightStartAge) end = rightStartAge;
	    }
	    if (start >= end) {
	  ...
	  TickBuffer.prototype._updateAmflowReceiveState = function () {
	    if (this._receiving && this._executionMode === ExecutionMode_1$3["default"].Passive) {
	  ...
	  TickBuffer.prototype._findNearestAbscentAge = function (age) {
	    ...
	    var len = this._tickRanges.length;
	    for (; i < len; ++i) {
	      if (age <= this._tickRanges[i].end) break;
	    }
	    for (; i < len; ++i) {
	      var range = this._tickRanges[i];
	      if (age < range.start) break;
	  ...
	  TickBuffer.prototype._dropUntil = function (age) {
	    ...
	    if (this._tickRanges.length === 0) return;
	    // start を書き換えることで、[start, age) の範囲を削除
	    var range = this._tickRanges[0];
	    if (age < range.start) return;
	    range.start = age;
	    for (i = 0; i < range.ticks.length; ++i) {
	      if (age <= range.ticks[i][0 /* EventIndex.Tick.Age */]) break;
	    }
	  ...
	  TickBuffer.prototype._calcKnownLatestTickTimeDelta = function (timeThreshold, baseTime, frameTime) {
	    ...
	    if (tickRanges.length === 0) return 0;
	    var timeDelta = 0;
	    ...
	    for (var i = tickRanges.length - 1; i >= 0; --i) {
	      ...
	      for (var j = ticksWithEvents.length - 1; j >= 0; --j) {
	        ...
	        var pevs = tick[1 /* EventIndex.Tick.Events */];
	        if (!pevs)
	          // tick がストレージ情報を持っていた時期との互換性のためのチェック。現在の game-driver が生成する tick ではこの条件を満たすことはない
	          continue;
	        // この tick までの時間を加算
	        var age = tick[0 /* EventIndex.Tick.Age */];
	        timeDelta += (lastAgeWithEvents - age) * frameTime;
	        if (timeDelta > timeThreshold)
	          // timestamp なしの tick だけで timeThreshold 分の時間を超過した
	...
	TickBuffer$1.TickBuffer = TickBuffer;

	var TickGenerator$1 = {};

	Object.defineProperty(TickGenerator$1, "__esModule", {
	  value: true
	});
	TickGenerator$1.TickGenerator = void 0;
	var g$3 = akashicEngine$1;
	/**
	 * `playlog.Tick` の生成器。
	...
	var TickGenerator = /** @class */function () {
	  function TickGenerator(param) {
	    this.tickTrigger = new g$3.Trigger();
	    this.errorTrigger = new g$3.Trigger();
	    this._nextAge = 0;
	    this._generatingTick = false;
	    if (param.errorHandler) this.errorTrigger.add(param.errorHandler, param.errorHandlerOwner);
	    this._amflow = param.amflow;
	    this._eventBuffer = param.eventBuffer;
	  }
	  TickGenerator.prototype.next = function () {
	    if (!this._generatingTick) return;
	    ...
	    var normalEvents = this._eventBuffer.readEvents();
	    var joinLeaves = this._eventBuffer.readJoinLeaves();
	    var evs = normalEvents && joinLeaves ? normalEvents.concat(joinLeaves) : normalEvents !== null && normalEvents !== void 0 ? normalEvents : joinLeaves;
	    this.tickTrigger.fire([this._nextAge++,
	    // 0: フレーム番号
	    evs // 1?: イベント
	    ]);
	  };
	...
	TickGenerator$1.TickGenerator = TickGenerator;

	Object.defineProperty(TickController$1, "__esModule", {
	  value: true
	});
	TickController$1.TickController = void 0;
	var g$2 = akashicEngine$1;
	var ExecutionMode_1$2 = ExecutionMode$1;
	var TickBuffer_1 = TickBuffer$1;
	var TickGenerator_1 = TickGenerator$1;
	/**
	...
	var TickController = /** @class */function () {
	  function TickController(param) {
	    this.errorTrigger = new g$2.Trigger();
	    if (param.errorHandler) this.errorTrigger.add(param.errorHandler, param.errorHandlerOwner);
	    this._amflow = param.amflow;
	  ...
	  TickController.prototype._updateGeneratorState = function () {
	    var toGenerate = this._started && this._executionMode === ExecutionMode_1$2["default"].Active;
	...
	TickController$1.TickController = TickController;

	Object.defineProperty(GameLoop$1, "__esModule", {
	  value: true
	});
	GameLoop$1.GameLoop = void 0;
	var g$1 = akashicEngine$1;
	var Clock_1 = Clock$1;
	var constants$1 = constants$2;
	var ExecutionMode_1$1 = ExecutionMode$1;
	var LoopMode_1$1 = LoopMode$1;
	var LoopRenderMode_1 = LoopRenderMode$1;
	var ProfilerClock_1 = ProfilerClock$1;
	var TickController_1 = TickController$1;
	g$1.EventIndex; // eslint-disable-line @typescript-eslint/naming-convention
	/**
	 * ゲームのメインループ管理クラス。
	...
	var GameLoop = /** @class */function () {
	  function GameLoop(param) {
	    this.errorTrigger = new g$1.Trigger();
	    this.rawTargetTimeReachedTrigger = new g$1.Trigger();
	    this.running = false;
	    /**
	    ...
	    this._delayIgnoreThreshold = conf.delayIgnoreThreshold || constants$1.DEFAULT_DELAY_IGNORE_THRESHOLD;
	    this._skipTicksAtOnce = conf.skipTicksAtOnce || constants$1.DEFAULT_SKIP_TICKS_AT_ONCE;
	    this._skipThreshold = conf.skipThreshold || constants$1.DEFAULT_SKIP_THRESHOLD;
	    this._skipThresholdTime = this._skipThreshold * this._frameTime;
	    // this._skipAwareGame はないことに注意 (Game#getIsSkipAware()) を使う
	    this._jumpTryThreshold = conf.jumpTryThreshold || constants$1.DEFAULT_JUMP_TRY_THRESHOLD;
	    this._jumpIgnoreThreshold = conf.jumpIgnoreThreshold || constants$1.DEFAULT_JUMP_IGNORE_THRESHOLD;
	    this._pollingTickThreshold = conf._pollingTickThreshold || constants$1.DEFAULT_POLLING_TICK_THRESHOLD;
	  ...
	  GameLoop.prototype.reset = function (startPoint) {
	    ...
	    if (this._skipping) this._stopSkipping();
	  ...
	  GameLoop.prototype.setLoopConfiguration = function (conf) {
	    if (conf.loopMode != null) this._loopMode = conf.loopMode;
	    if (conf.delayIgnoreThreshold != null) this._delayIgnoreThreshold = conf.delayIgnoreThreshold;
	    if (conf.skipTicksAtOnce != null) this._skipTicksAtOnce = conf.skipTicksAtOnce;
	    if (conf.skipThreshold != null) {
	      this._skipThreshold = conf.skipThreshold;
	      this._skipThresholdTime = this._skipThreshold * this._frameTime;
	    }
	    if (conf.skipAwareGame != null) this._game.setIsSkipAware(conf.skipAwareGame);
	    if (conf.jumpTryThreshold != null) this._jumpTryThreshold = conf.jumpTryThreshold;
	    if (conf.jumpIgnoreThreshold != null) this._jumpIgnoreThreshold = conf.jumpIgnoreThreshold;
	    if (conf.playbackRate != null) {
	      this._playbackRate = conf.playbackRate;
	      this._clock.changeScaleFactor(this._playbackRate);
	      this._updateGameAudioSuppression();
	    }
	    if (conf.loopRenderMode != null) this._setLoopRenderMode(conf.loopRenderMode);
	    if (conf.targetTimeFunc != null) {
	      this._targetTimeFunc = conf.targetTimeFunc;
	    }
	    if (conf.targetTimeOffset != null) this._targetTimeOffset = conf.targetTimeOffset;
	    if (conf.originDate != null) this._originDate = conf.originDate;
	    this._realTargetTimeOffset = this._originDate != null ? this._originDate : (this._targetTimeOffset || 0) + this._startedAt;
	    if (conf.omitInterpolatedTickOnReplay != null) this._omitInterpolatedTickOnReplay = conf.omitInterpolatedTickOnReplay;
	  ...
	  GameLoop.prototype._startSkipping = function (isNear) {
	    this._skipping = true;
	    if (!isNear) this._updateGameAudioSuppression();
	  ...
	  GameLoop.prototype._onFrame = function (frameArg) {
	    if (this._loopMode !== LoopMode_1$1["default"].Replay || !this._targetTimeFunc) {
	      ...
	      if (prevTickTime === this._currentTickTime && prevTickTime <= targetTime && this._isImmediateBeforeOf(targetTime)) this.rawTargetTimeReachedTrigger.fire(givenTargetTime);
	    }
	  };
	  /**
	  ...
	  GameLoop.prototype._onFrameForTimedReplay = function (targetTime, frameArg) {
	    ...
	    if ((frameGap > this._jumpTryThreshold || frameGap < 0) && !this._waitingStartPoint && this._lastRequestedStartPointTime < this._currentTickTime) {
	    ...
	    if (!this._skipping) {
	      if ((frameGap > this._skipThreshold || this._tickBuffer.currentAge === 0) && (this._tickBuffer.hasNextTick() || this._omitInterpolatedTickOnReplay && this._foundLatestTick)) {
	    ...
	    for (; consumedFrame < this._skipTicksAtOnce; ++consumedFrame) {
	      ...
	      if (!this._tickBuffer.hasNextTick()) {
	        if (!this._waitingNextTick) {
	          ...
	          if (!this._foundLatestTick) this._tickBuffer.requestNonIgnorableTicks();
	        }
	        if (this._omitInterpolatedTickOnReplay && this._sceneLocalMode === "interpolate-local") {
	          ...
	          if (targetTime > nextFrameTime) this._doLocalTick();
	        }
	        break;
	      }
	  ...
	  GameLoop.prototype._onFrameNormal = function (frameArg) {
	    ...
	    if (!this._skipping && frameArg.deltaTime > this._skipThresholdTime) {
	      ...
	      if (this._waitingNextTick) this._stopSkipping();
	    }
	    if (this._waitingNextTick) {
	      if (this._sceneLocalMode === "interpolate-local") this._doLocalTick();
	      return;
	    }
	    var targetAge;
	    var ageGap;
	    var currentAge = this._tickBuffer.currentAge;
	    if (this._loopMode === LoopMode_1$1["default"].Realtime) {
	    ...
	    if (!this._skipping && (ageGap > this._skipThreshold || currentAge === 0) && this._tickBuffer.hasNextTick()) {
	      ...
	      var isTargetNear = currentAge === 0 &&
	      // 余計な関数呼び出しを避けるためにチェック
	    ...
	    for (; consumedFrame < loopCount; ++consumedFrame) {
	      ...
	      if (explicitNextTickTime != null && nextFrameTime < explicitNextTickTime) {
	        if (this._loopMode === LoopMode_1$1["default"].Realtime || this._omitInterpolatedTickOnReplay && this._skipping) {
	    ...
	    // @ts-ignore TODO: targetAge が null の場合の振る舞い
	    if (this._skipping && targetAge - this._tickBuffer.currentAge < 1) this._stopSkipping();
	  };
	  GameLoop.prototype._onGotNextFrameTick = function () {
	    if (!this._waitingNextTick) return;
	    if (this._loopMode === LoopMode_1$1["default"].FrameByFrame) {
	  ...
	  GameLoop.prototype._onGotNoTick = function () {
	    if (this._waitingNextTick) this._foundLatestTick = true;
	  };
	  GameLoop.prototype._onGotStartPoint = function (err, startPoint) {
	    ...
	    if (!this._targetTimeFunc || this._loopMode === LoopMode_1$1["default"].Realtime) {
	      var targetAge = this._loopMode === LoopMode_1$1["default"].Realtime ? this._tickBuffer.knownLatestAge + 1 : this._targetAge;
	  ...
	  GameLoop.prototype._setLoopRenderMode = function (mode) {
	    if (mode === this._loopRenderMode) return;
	  ...
	  GameLoop.prototype._onGameRaiseTick = function (es) {
	    if (this._executionMode !== ExecutionMode_1$1["default"].Active) return;
	    // TODO: イベントフィルタの中で呼ばれるとおかしくなる(フィルタ中のイベントがtickに乗らない)。
	    if (es) {
	      for (var i = 0; i < es.length; ++i) this._eventBuffer.addEventDirect(es[i]);
	    }
	  ...
	  GameLoop.prototype._isImmediateBeforeOf = function (targetTime) {
	    ...
	    return this._tickBuffer.hasNextTick() ? targetTime < ((_a = this._tickBuffer.readNextTickTime()) !== null && _a !== void 0 ? _a : this._currentTickTime + this._frameTime) : this._foundLatestTick;
	  };
	  return GameLoop;
	}();
	GameLoop$1.GameLoop = GameLoop;

	Object.defineProperty(GameDriver$1, "__esModule", {
	  value: true
	});
	GameDriver$1.GameDriver = void 0;
	var g = akashicEngine$1;
	var utils_1 = utils$1;
	var constants = constants$2;
	var EventBuffer_1 = EventBuffer$1;
	var ExecutionMode_1 = ExecutionMode$1;
	var Game_1 = Game$1;
	var GameHandlerSet_1 = GameHandlerSet$1;
	var GameLoop_1 = GameLoop$1;
	var LoopMode_1 = LoopMode$1;
	...
	var GameDriver = /** @class */function () {
	  ...
	  GameDriver.prototype.changeState = function (param, callback) {
	    ...
	    if (pausing) (_a = this._gameLoop) === null || _a === void 0 ? void 0 : _a.stop();
	    this.initialize(param, function (err) {
	      ...
	      if (pausing) (_a = _this._gameLoop) === null || _a === void 0 ? void 0 : _a.start();
	  ...
	  GameDriver.prototype.resetPrimarySurface = function (width, height, rendererCandidates) {
	    rendererCandidates = rendererCandidates ? rendererCandidates : this._rendererRequirement ? this._rendererRequirement.rendererCandidates : undefined;
	    var game = this._game;
	  ...
	  GameDriver.prototype._doSetDriverConfiguration = function (dconf) {
	    ...
	    // デフォルト値の補完
	    if (dconf.playId === undefined) dconf.playId = (_a = this._playId) !== null && _a !== void 0 ? _a : undefined;
	    if (dconf.playToken === undefined) dconf.playToken = (_b = this._playToken) !== null && _b !== void 0 ? _b : undefined;
	    if (dconf.eventBufferMode === undefined) {
	      if (dconf.executionMode === ExecutionMode_1["default"].Active) {
	        dconf.eventBufferMode = {
	          isReceiver: true,
	          isSender: false
	        };
	    ...
	    return p.then(function () {
	      ...
	      return new Promise(function (resolve, reject) {
	        ...
	        _this._platform.amflow.open(playId, function (err) {
	          ...
	          if (_this._game) _this._updateGamePlayId(_this._game);
	          resolve();
	        });
	      });
	    });
	  ...
	  GameDriver.prototype._doAuthenticate = function (playToken) {
	    var _this = this;
	    if (playToken == null) return Promise.resolve();
	    return new Promise(function (resolve, reject) {
	      _this._platform.amflow.authenticate(playToken, function (err, permission) {
	        ...
	        if (_this._game) {
	          _this._game.rawHandlerSet.isSnapshotSaver = permission.writeTick;
	        }
	        resolve();
	      });
	    });
	  ...
	  GameDriver.prototype._getStartPoint = function (frame) {
	    ...
	    return new Promise(function (resolve, reject) {
	      _this._platform.amflow.getStartPoint({
	        ...
	        if (!startPoint) return reject(new Error("GameDriver#_getStartPoint: No startPoint found"));
	        resolve(startPoint);
	      });
	    });
	  };
	  GameDriver.prototype._createGame = function (conf, player, param) {
	    ...
	    if (!param.loopConfiguration)
	      // このパス (configurationUrl があって Game を作る) では必須
	    ...
	    p = p.then(function () {
	      ...
	      if (!putSeed && loopConfiguration.loopMode === LoopMode_1["default"].Realtime) {
	        ...
	        return _this._getStartPoint(constants.PSEUDO_INFINITE_AGE).then(function (latestSp) {
	          return [zerothSp, latestSp];
	        });
	      }
	      return [zerothSp, zerothSp];
	    });
	    return p.then(function (_a) {
	      var zerothStartPoint = _a[0],
	        latestStartPoint = _a[1];
	      _this._assertLive();
	      var zerothData = zerothStartPoint.data;
	      if (typeof zerothData.seed !== "number")
	        // 型がないので一応確認
	      ...
	      var eventBuffer = new EventBuffer_1.EventBuffer({
	        game: game,
	        amflow: pf.amflow
	      });
	      ...
	      if (_this._hidden) game._setMuted(true);
	...
	GameDriver$1.GameDriver = GameDriver;
	...
	(function (exports) {
	  ...
	  var MemoryAmflowClient_1 = MemoryAmflowClient$1;
	  Object.defineProperty(exports, "MemoryAmflowClient", {
	    enumerable: true,
	    get: function get() {
	      return MemoryAmflowClient_1.MemoryAmflowClient;
	    }
	  });
	  var ReplayAmflowProxy_1 = ReplayAmflowProxy$1;
	  Object.defineProperty(exports, "ReplayAmflowProxy", {
	    enumerable: true,
	    get: function get() {
	      return ReplayAmflowProxy_1.ReplayAmflowProxy;
	    }
	  });
	  var SimpleProfiler_1 = SimpleProfiler$1;
	  Object.defineProperty(exports, "SimpleProfiler", {
	    enumerable: true,
	    get: function get() {
	      return SimpleProfiler_1.SimpleProfiler;
	    }
	  });
	  var ExecutionMode_1 = ExecutionMode$1;
	  ...
	  var GameDriver_1 = GameDriver$1;
	  Object.defineProperty(exports, "GameDriver", {
	    enumerable: true,
	    get: function get() {
	      return GameDriver_1.GameDriver;
	    }
	  });
	  var LoopMode_1 = LoopMode$1;
	...
	var gameDriver = lib$3;

	var AudioAsset$2 = {};

	var Asset$2 = {};

	Object.defineProperty(Asset$2, "__esModule", {
	  value: true
	});
	...
	var Asset$1 = /** @class */function () {
	  function Asset(id, path) {
	    ...
	    this.onDestroyed = new trigger_1$1.Trigger();
	  }
	...
	Object.defineProperty(AudioAsset$2, "__esModule", {
	  value: true
	});
	AudioAsset$2.AudioAsset = void 0;
	var Asset_1$2 = Asset$2;
	/**
	 * 音リソースを表すクラス。
	 * 本クラスのインスタンスをゲーム開発者が直接生成することはない。
	 * game.jsonによって定義された内容をもとに暗黙的に生成されたインスタンスを、
	...
	AudioAsset$2.AudioAsset = AudioAsset$1;

	var AudioPlayer$2 = {};

	Object.defineProperty(AudioPlayer$2, "__esModule", {
	  value: true
	});
	AudioPlayer$2.AudioPlayer = void 0;
	var trigger_1 = cjs;
	/**
	 * サウンド再生を行うクラス。
	 *
	...
	AudioPlayer$2.AudioPlayer = AudioPlayer$1;
	...
	Object.defineProperty(ScriptAsset$1, "__esModule", {
	  value: true
	});
	ScriptAsset$1.ScriptAsset = void 0;
	var Asset_1$1 = Asset$2;
	/**
	 * スクリプトリソースを表すクラス。
	 * 本クラスのインスタンスをゲーム開発者が直接生成することはない。
	 * game.jsonによって定義された内容をもとに暗黙的に生成されたインスタンスを、
	...
	var ScriptAsset = /** @class */function (_super) {
	  __extends$1(ScriptAsset, _super);
	...
	Object.defineProperty(TextAsset$1, "__esModule", {
	  value: true
	});
	...
	var akashicEngine = akashicEngine$1;
	...
	akashicEngine.AudioAsset = AudioAsset$2.AudioAsset;
	akashicEngine.AudioPlayer = AudioPlayer$2.AudioPlayer;
	akashicEngine.ScriptAsset = ScriptAsset$1.ScriptAsset;
	akashicEngine.TextAsset = TextAsset$1.TextAsset;
	var akashicEngine_1 = akashicEngine;
	...
	function requireExceptionFactory() {
	  ...
	  Object.defineProperty(ExceptionFactory, "__esModule", {
	    value: true
	  });
	...
	function requireXHRLoader() {
	  ...
	  Object.defineProperty(XHRLoader, "__esModule", {
	    value: true
	  });
	...
	function requireXHRTextAsset() {
	  ...
	  Object.defineProperty(XHRTextAsset, "__esModule", {
	    value: true
	  });
	...
	function requireAudioManager() {
	  ...
	  Object.defineProperty(AudioManager, "__esModule", {
	    value: true
	  });
	  ...
	  var AudioManager$1 = /** @class */function () {
	    ...
	    AudioManager.prototype.registerAudioAsset = function (asset) {
	      if (this.audioAssets.indexOf(asset) === -1) this.audioAssets.push(asset);
	    };
	    AudioManager.prototype.removeAudioAsset = function (asset) {
	      var index = this.audioAssets.indexOf(asset);
	      if (index === -1) this.audioAssets.splice(index, 1);
	    };
	...
	function requireInputEventHandler() {
	  ...
	  Object.defineProperty(InputEventHandler, "__esModule", {
	    value: true
	  });
	  ...
	  var InputEventHandler$1 = /** @class */function () {
	    ...
	    InputEventHandler.prototype.pointDown = function (identifier, pagePosition, button) {
	      ...
	      if (pagePosition.offsetX < 0 || pagePosition.offsetY < 0 || pagePosition.offsetX > this.inputView.offsetWidth || pagePosition.offsetY > this.inputView.offsetHeight) return;
	...
	function requireMouseTouchEventHandler() {
	  ...
	  Object.defineProperty(MouseTouchEventHandler, "__esModule", {
	    value: true
	  });
	  ...
	  var MouseTouchEventHandler$1 = /** @class */function (_super) {
	    ...
	    function MouseTouchEventHandler() {
	      ...
	      _this.onMouseDown = function (e) {
	        ...
	        if (_this.pressingMouseButton != null) return;
	      ...
	      _this.onWindowMouseUp = function (e) {
	        if (_this.pressingMouseButton !== e.button) return;
	        _this.pressingMouseButton = null;
	...
	function requirePointerEventHandler() {
	  ...
	  Object.defineProperty(PointerEventHandler, "__esModule", {
	    value: true
	  });
	      ...
	      _this.onPointerDown = function (e) {
	        ...
	        var onPointerUp = function onPointerUp(event) {
	          ...
	          if (e.pointerId === event.pointerId) {
	            ...
	            if (!handlers) return;
	            var onPointerMove_1 = handlers.onPointerMove,
	              onPointerUp_1 = handlers.onPointerUp;
	        ...
	        _this._eventHandlersMap[e.pointerId] = {
	...
	function requireInputHandlerLayer() {
	  ...
	  Object.defineProperty(InputHandlerLayer, "__esModule", {
	    value: true
	  });
	...
	function requireContainerController() {
	  ...
	  Object.defineProperty(ContainerController, "__esModule", {
	    value: true
	  });
	  ...
	  var ContainerController$1 = /** @class */function () {
	    ...
	    ContainerController.prototype._loadView = function () {
	      var _a = this._rendererReq,
	        width = _a.primarySurfaceWidth,
	        height = _a.primarySurfaceHeight;
	      // DocumentFragmentはinsertした時点で開放されているため毎回作る
	      // https://dom.spec.whatwg.org/#concept-node-insert
	      this.container = document.createDocumentFragment();
	      // 入力受け付けレイヤー - DOM Eventの管理
	      if (!this.inputHandlerLayer) {
	        this.inputHandlerLayer = new InputHandlerLayer_1.InputHandlerLayer({
	          width: width,
	          height: height
	        });
	      } else {
	...
	function requireAudioPluginManager() {
	  ...
	  Object.defineProperty(AudioPluginManager, "__esModule", {
	    value: true
	  });
	...
	function requireAudioPluginRegistry() {
	  ...
	  Object.defineProperty(AudioPluginRegistry, "__esModule", {
	    value: true
	  });
	  ...
	  AudioPluginRegistry.AudioPluginRegistry = {
	    ...
	    getRegisteredAudioPlugins: function getRegisteredAudioPlugins() {
	      return audioPlugins;
	    },
	...
	function requireRafLooper() {
	  ...
	  Object.defineProperty(RafLooper, "__esModule", {
	    value: true
	  });
	  ...
	  var RafLooper$1 = /** @class */function () {
	    ...
	    RafLooper.prototype.start = function () {
	      var _this = this;
	      var onAnimationFrame = function onAnimationFrame(deltaTime) {
	      ...
	      var onFirstFrame = function onFirstFrame(deltaTime) {
	        _this._timerId = requestAnimationFrame(onAnimationFrame);
	        _this._fun(0);
	        _this._prev = deltaTime;
	      };
	...
	function requireBinaryAsset() {
	  ...
	  Object.defineProperty(BinaryAsset, "__esModule", {
	    value: true
	  });
	...
	function requireSurface() {
	  ...
	  Object.defineProperty(Surface, "__esModule", {
	    value: true
	  });
	...
	function requireCanvasSurface() {
	  ...
	  Object.defineProperty(CanvasSurface, "__esModule", {
	    value: true
	  });
	  CanvasSurface.CanvasSurface = void 0;
	  var Surface_1 = requireSurface();
	...
	function requireAffineTransformer() {
	  ...
	  Object.defineProperty(AffineTransformer, "__esModule", {
	    value: true
	  });
	...
	function requireCanvasRenderingState() {
	  ...
	  Object.defineProperty(CanvasRenderingState, "__esModule", {
	    value: true
	  });
	...
	function requireCanvasSurfaceContext() {
	  ...
	  Object.defineProperty(CanvasSurfaceContext, "__esModule", {
	    value: true
	  });
	...
	function requireContext2DRenderer() {
	  ...
	  Object.defineProperty(Context2DRenderer, "__esModule", {
	    value: true
	  });
	...
	function requireContext2DSurface() {
	  ...
	  Object.defineProperty(Context2DSurface, "__esModule", {
	    value: true
	  });
	...
	function requireSVGImageAsset() {
	  ...
	  Object.defineProperty(SVGImageAsset, "__esModule", {
	    value: true
	  });
	  SVGImageAsset.SVGImageAsset = SVGImageAsset.SVGImageAssetSurface = void 0;
	  ...
	  SVGImageAsset.SVGImageAssetSurface = SVGImageAssetSurface;
	  var SVGImageAsset$1 = /** @class */function (_super) {
	    ...
	    SVGImageAsset.prototype.createSurface = function (width, height, sx, sy, sWidth, sHeight) {
	      ...
	      var _a = this,
	        viewportWidth = _a.width,
	        viewportHeight = _a.height,
	        data = _a.data;
	      if (!data) {
	  ...
	  SVGImageAsset.SVGImageAsset = SVGImageAsset$1;
	  return SVGImageAsset;
	}
	...
	function requireGeneratedSVGImageAsset() {
	  ...
	  Object.defineProperty(GeneratedSVGImageAsset, "__esModule", {
	    value: true
	  });
	...
	function requireHTMLImageAsset() {
	  ...
	  Object.defineProperty(HTMLImageAsset, "__esModule", {
	    value: true
	  });
	...
	function requireHTMLVideoPlayer() {
	  ...
	  Object.defineProperty(HTMLVideoPlayer, "__esModule", {
	    value: true
	  });
	...
	function requireHTMLVideoAsset() {
	  ...
	  Object.defineProperty(HTMLVideoAsset, "__esModule", {
	    value: true
	  });
	...
	function requireXHRScriptAsset() {
	  ...
	  Object.defineProperty(XHRScriptAsset, "__esModule", {
	    value: true
	  });
	...
	function requireGlyphFactory() {
	  ...
	  Object.defineProperty(GlyphFactory, "__esModule", {
	    value: true
	  });
	  ...
	  function createGlyphRenderedSurface(code, fontSize, cssFontFamily, baselineHeight, marginW, marginH, needImageData, fontColor, strokeWidth, strokeColor, strokeOnly, fontWeight) {
	    ...
	    var scale = fontSize < GlyphFactory$1._environmentMinimumFontSize ? fontSize / GlyphFactory$1._environmentMinimumFontSize : 1;
	  ...
	  var genericFontFamilyNames = ["serif", "sans-serif", "monospace", "cursive", "fantasy", "system-ui"];
	  ...
	  var GlyphFactory$1 = /** @class */function () {
	    function GlyphFactory(fontFamily, fontSize, baselineHeight, fontColor, strokeWidth, strokeColor, strokeOnly, fontWeight) {
	      ...
	      this._cssFontFamily = typeof fontFamily === "string" ? quoteIfNonGeneric(fontFamily) : fontFamily.map(quoteIfNonGeneric).join(",");
	...
	function requireRenderingHelper() {
	  ...
	  Object.defineProperty(RenderingHelper, "__esModule", {
	    value: true
	  });
	...
	function requireWebGLColor() {
	  ...
	  Object.defineProperty(WebGLColor, "__esModule", {
	    value: true
	  });
	  ...
	  (function (WebGLColor) {
	    ...
	    function _toColor(cssColor) {
	      ...
	      if (ncc.match(/^#([\dA-F])([\dA-F])([\dA-F])$/)) {
	        return [parseInt(RegExp.$1, 16) / 15.0, parseInt(RegExp.$2, 16) / 15.0, parseInt(RegExp.$3, 16) / 15.0, 1.0];
	      } else if (ncc.match(/^#([\dA-F]{2})([\dA-F]{2})([\dA-F]{2})$/)) {
	        return [parseInt(RegExp.$1, 16) / 255.0, parseInt(RegExp.$2, 16) / 255.0, parseInt(RegExp.$3, 16) / 255.0, 1.0];
	      } else if (ncc.match(/^RGB\((\d{1,3}),(\d{1,3}),(\d{1,3})\)$/)) {
	        return [parseInt(RegExp.$1, 10) / 255.0, parseInt(RegExp.$2, 10) / 255.0, parseInt(RegExp.$3, 10) / 255.0, 1.0];
	      } else if (ncc.match(/^RGBA\((\d{1,3}),(\d{1,3}),(\d{1,3}),(\d(\.\d*)?)\)$/)) {
	        return [parseInt(RegExp.$1, 10) / 255.0, parseInt(RegExp.$2, 10) / 255.0, parseInt(RegExp.$3, 10) / 255.0, parseFloat(RegExp.$4)];
	      } else if (ncc.match(/^HSL\((\d{1,3}),(\d{1,3})%,(\d{1,3})%\)$/)) {
	        return WebGLColor._hsl2rgb([parseInt(RegExp.$1, 10), RenderingHelper_1.RenderingHelper.clamp(parseInt(RegExp.$2, 10) / 100.0), RenderingHelper_1.RenderingHelper.clamp(parseInt(RegExp.$3, 10) / 100.0), 1.0]);
	      } else if (ncc.match(/^HSLA\((\d{1,3}),(\d{1,3})%,(\d{1,3})%,(\d(\.\d*)?)\)$/)) {
	        return WebGLColor._hsl2rgb([parseInt(RegExp.$1, 10), RenderingHelper_1.RenderingHelper.clamp(parseInt(RegExp.$2, 10) / 100.0), RenderingHelper_1.RenderingHelper.clamp(parseInt(RegExp.$3, 10) / 100.0), parseFloat(RegExp.$4)]);
	      } else {
	...
	function requireWebGLRenderingState() {
	  ...
	  Object.defineProperty(WebGLRenderingState, "__esModule", {
	    value: true
	  });
	...
	function requireWebGLRenderer() {
	  ...
	  Object.defineProperty(WebGLRenderer, "__esModule", {
	    value: true
	  });
	...
	function requireWebGLBackSurfaceRenderer() {
	  ...
	  Object.defineProperty(WebGLBackSurfaceRenderer, "__esModule", {
	    value: true
	  });
	...
	function requireWebGLPrimarySurfaceRenderer() {
	  ...
	  Object.defineProperty(WebGLPrimarySurfaceRenderer, "__esModule", {
	    value: true
	  });
	...
	function requireWebGLPrimarySurface() {
	  ...
	  Object.defineProperty(WebGLPrimarySurface, "__esModule", {
	    value: true
	  });
	...
	function requireWebGLBackSurface() {
	  ...
	  Object.defineProperty(WebGLBackSurface, "__esModule", {
	    value: true
	  });
	...
	function requireWebGLShaderProgram() {
	  ...
	  Object.defineProperty(WebGLShaderProgram, "__esModule", {
	    value: true
	  });
	  ...
	  var WebGLShaderProgram$1 = /** @class */function () {
	    ...
	    WebGLShaderProgram.prototype.updateUniforms = function () {
	      for (var i = 0; i < this._uniformCaches.length; ++i) {
	        ...
	        if (!cache.isArray && value === cache.beforeValue) continue;
	...
	function requireWebGLTextureMap() {
	  ...
	  Object.defineProperty(WebGLTextureMap, "__esModule", {
	    value: true
	  });
	...
	function requireWebGLTextureAtlas() {
	  ...
	  Object.defineProperty(WebGLTextureAtlas, "__esModule", {
	    value: true
	  });
	...
	function requireWebGLSharedObject() {
	  ...
	  Object.defineProperty(WebGLSharedObject, "__esModule", {
	    value: true
	  });
	  ...
	  var WebGLSharedObject$1 = /** @class */function () {
	    function WebGLSharedObject(params) {
	      ...
	      var context = surface.canvas.getContext("webgl", {
	        depth: enableDepthBuffer,
	...
	function requireCanvasDisposer() {
	  ...
	  Object.defineProperty(CanvasDisposer, "__esModule", {
	    value: true
	  });
	...
	function requireSurfaceFactory() {
	  ...
	  Object.defineProperty(SurfaceFactory, "__esModule", {
	    value: true
	  });
	  ...
	  var SurfaceFactory$1 = /** @class */function () {
	    ...
	    SurfaceFactory.prototype.createPrimarySurface = function (width, height, rendererCandidates) {
	      ...
	      if (usedWebGL) {
	        if (!this._shared) {
	          this._shared = new WebGLSharedObject_1.WebGLSharedObject({
	            width: width,
	            height: height,
	            enableDepthBuffer: usedWebGL.options.enableDepthBuffer
	          });
	        }
	    ...
	    SurfaceFactory.prototype.createBackSurface = function (width, height, rendererCandidates) {
	      var surface = RenderingHelper_1.RenderingHelper.usedWebGL(rendererCandidates) ? this._shared.createBackSurface(width, height) : new Context2DSurface_1.Context2DSurface(width, height);
	...
	function requireResourceFactory() {
	  if (hasRequiredResourceFactory) return ResourceFactory;
	  hasRequiredResourceFactory = 1;
	  Object.defineProperty(ResourceFactory, "__esModule", {
	    value: true
	  });
	  ResourceFactory.ResourceFactory = void 0;
	  var BinaryAsset_1 = requireBinaryAsset();
	...
	function requirePlatform() {
	  ...
	  var Platform$1 = /** @class */function () {
	    ...
	    Platform.prototype.loadGameConfiguration = function (url, callback) {
	      ...
	      a._load({
	        _onAssetLoad: function _onAssetLoad(_asset) {
	          callback(null, JSON.parse(a.data));
	        },
	        _onAssetError: function _onAssetError(_asset, error) {
	          callback(error, null);
	        }
	      });
	    };
	    ...
	    Platform.prototype.setRendererRequirement = function (requirement) {
	      var _a;
	      if (!requirement) {
	        if (this.containerController) this.containerController.unloadView();
	        return;
	      }
	...
	function requireAudioUtil() {
	  ...
	  Object.defineProperty(audioUtil, "__esModule", {
	    value: true
	  });
	  ...
	  function resolveExtname(extensions, supportedFormats) {
	    ...
	    for (var _i = 0, supportedFormats_1 = supportedFormats; _i < supportedFormats_1.length; _i++) {
	      ...
	      var ext = "." + fmt;
	      if (extensions.indexOf(ext) !== -1) return ext;
	    }
	    return null;
	  }
	...
	function requireAudioAsset() {
	  ...
	  Object.defineProperty(AudioAsset, "__esModule", {
	    value: true
	  });
	...
	function requireHTMLAudioAsset() {
	  ...
	  Object.defineProperty(HTMLAudioAsset, "__esModule", {
	    value: true
	  });
	  ...
	  var HTMLAudioAsset$1 = /** @class */function (_super) {
	    ...
	    HTMLAudioAsset.prototype._load = function (loader) {
	      ...
	      var startLoadingAudio = function startLoadingAudio(path, handlers) {
	      ...
	      var setAudioLoadInterval = function setAudioLoadInterval(audio, handlers) {
	...
	function requireAudioPlayer() {
	  ...
	  Object.defineProperty(AudioPlayer, "__esModule", {
	    value: true
	  });
	...
	function requireHTMLAudioAutoplayHelper() {
	  ...
	  Object.defineProperty(HTMLAudioAutoplayHelper, "__esModule", {
	    value: true
	  });
	    ...
	    function playHandler() {
	      switch (state) {
	        ...
	        case 1 /* PlayableState.WaitingInteraction */:
	          // 通常のケースではここには到達しないが、何らかの外因によって音を鳴らすことができた場合
	...
	function requireHTMLAudioPlayer() {
	  ...
	  Object.defineProperty(HTMLAudioPlayer, "__esModule", {
	    value: true
	  });
	  ...
	  var HTMLAudioPlayer$1 = /** @class */function (_super) {
	    ...
	    HTMLAudioPlayer.prototype._clearEndedEventHandler = function () {
	      if (this._audioInstance) this._audioInstance.removeEventListener("ended", this._endedEventHandler, false);
	...
	function requireHTMLAudioPlugin() {
	  ...
	  Object.defineProperty(HTMLAudioPlugin, "__esModule", {
	    value: true
	  });
	  ...
	  var HTMLAudioPlugin$1 = /** @class */function () {
	    ...
	    Object.defineProperty(HTMLAudioPlugin.prototype, "supportedFormats", {
	      ...
	      set: function set(supportedFormats) {
	...
	function requireProxyAudioAsset() {
	  ...
	  Object.defineProperty(ProxyAudioAsset, "__esModule", {
	    value: true
	  });
	...
	function requireProxyAudioPlayer() {
	  ...
	  Object.defineProperty(ProxyAudioPlayer, "__esModule", {
	    value: true
	  });
	...
	function requireProxyAudioPlugin() {
	  ...
	  Object.defineProperty(ProxyAudioPlugin, "__esModule", {
	    value: true
	  });
	...
	function requireWebAudioAsset() {
	  ...
	  Object.defineProperty(WebAudioAsset, "__esModule", {
	    value: true
	  });
	  ...
	  var WebAudioAsset$1 = /** @class */function (_super) {
	    ...
	    WebAudioAsset.prototype._load = function (loader) {
	      ...
	      var successHandler = function successHandler(decodedAudio) {
	      ...
	      var onLoadArrayBufferHandler = function onLoadArrayBufferHandler(response) {
	      ...
	      var loadArrayBuffer = function loadArrayBuffer(path, onSuccess, onFailed) {
	...
	function requireWebAudioPlayer() {
	  ...
	  Object.defineProperty(WebAudioPlayer, "__esModule", {
	    value: true
	  });
	  ...
	  var WebAudioPlayer$1 = /** @class */function (_super) {
	    ...
	    WebAudioPlayer.prototype.stop = function () {
	      ...
	      this._clearEndedEventHandler();
	      if (this._sourceNode) this._sourceNode.stop(0);
	      _super.prototype.stop.call(this);
	    };
	...
	function requireWebAudioPlugin() {
	  ...
	  Object.defineProperty(WebAudioPlugin, "__esModule", {
	    value: true
	  });
	...
	function requireFull() {
	  ...
	  (function (exports) {
	    ...
	    Object.defineProperty(exports, "Platform", {
	      enumerable: true,
	      get: function get() {
	        return Platform_1.Platform;
	      }
	    });
	    var AudioPluginRegistry_1 = requireAudioPluginRegistry();
	    Object.defineProperty(exports, "AudioPluginRegistry", {
	      enumerable: true,
	      get: function get() {
	        return AudioPluginRegistry_1.AudioPluginRegistry;
	      }
	    });
	    var AudioPluginManager_1 = requireAudioPluginManager();
	    Object.defineProperty(exports, "AudioPluginManager", {
	      enumerable: true,
	      get: function get() {
	        return AudioPluginManager_1.AudioPluginManager;
	      }
	    });
	    // TODO: Audio Pluginの実態は別リポジトリに切り出す事を検討する
	    var HTMLAudioPlugin_1 = requireHTMLAudioPlugin();
	    Object.defineProperty(exports, "HTMLAudioPlugin", {
	      enumerable: true,
	      get: function get() {
	        return HTMLAudioPlugin_1.HTMLAudioPlugin;
	      }
	    });
	    var ProxyAudioPlugin_1 = requireProxyAudioPlugin();
	    Object.defineProperty(exports, "ProxyAudioPlugin", {
	      enumerable: true,
	      get: function get() {
	        return ProxyAudioPlugin_1.ProxyAudioPlugin;
	      }
	    });
	    var WebAudioPlugin_1 = requireWebAudioPlugin();
	    Object.defineProperty(exports, "WebAudioPlugin", {
	      enumerable: true,
	      get: function get() {
	        return WebAudioPlugin_1.WebAudioPlugin;
	      }
	    });
	    var ResourceFactory_1 = requireResourceFactory();
	    Object.defineProperty(exports, "ResourceFactory", {
	      enumerable: true,
	      get: function get() {
	        return ResourceFactory_1.ResourceFactory;
	      }
	    });
	  })(full);
	  return full;
	...
	var engineFiles = {
	  ...
	  akashicEngine: akashicEngine_1,
	  pdiBrowser: typeof window !== "undefined" ? requirePdiBrowser() : null
	};
	var engineFiles$1 = /*@__PURE__*/getDefaultExportFromCjs(engineFiles);

	return engineFiles$1;
